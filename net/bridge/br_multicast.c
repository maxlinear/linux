...
static void
__br_multicast_port_send_query_explicit(struct net_bridge_mcast_port *pmctx,
					__be16 proto)
{
	struct net_bridge_mcast *brmctx;
	struct br_ip group = {};
	...
	__br_multicast_send_query(brmctx, pmctx, NULL, NULL, &group, false,
				  0, NULL);
}

static void br_ip4_multicast_port_send_query_explicit(struct timer_list *t)
{
	struct net_bridge_mcast_port *pmctx = from_timer(pmctx, t,
							 ip4_sta_query.timer);

	spin_lock(&pmctx->port->br->multicast_lock);
	__br_multicast_port_send_query_explicit(pmctx, ETH_P_IP);
	pmctx->ip4_sta_query.delay_time = 0;
	spin_unlock(&pmctx->port->br->multicast_lock);
}

#if IS_ENABLED(CONFIG_IPV6)
static void br_ip6_multicast_port_send_query_explicit(struct timer_list *t)
{
	struct net_bridge_mcast_port *pmctx = from_timer(pmctx, t,
							 ip6_sta_query.timer);

	spin_lock(&pmctx->port->br->multicast_lock);
	__br_multicast_port_send_query_explicit(pmctx, ETH_P_IPV6);
	pmctx->ip6_sta_query.delay_time = 0;
	spin_unlock(&pmctx->port->br->multicast_lock);
}

#define IN6_ADDR_IS_MULTICAST(a) (((__const uint8_t *) (a))[0] == 0xff)
#define IN6_ADDR_IS_MC_GLOBAL(a)					\
	(IN6_ADDR_IS_MULTICAST(a)					\
	 && ((((const uint8_t *) (a))[1] & 0xf) == 0xe))

#endif

static bool br_port_is_accelerable_group(struct br_ip *group)
{
	if (group->proto == htons(ETH_P_IP)) {
		/* IPv4 link-local multicast space 224.0.0.0/24 and
		 * IPv4 Service Discovery Protocol address 239.255.255.250
		 */
		if (ipv4_is_local_multicast(group->dst.ip4) ||
		    (group->dst.ip4 == htonl(0xEFFFFFFAU))) {
			return false;
		}
#if IS_ENABLED(CONFIG_IPV6)
	} else if (group->proto == htons(ETH_P_IPV6)) {
		struct in6_addr *addr = &group->dst.ip6;

		/* Validate IPv6 address - Accelerable or not */
		if (!IN6_ADDR_IS_MC_GLOBAL(addr->s6_addr))
			return false;
#endif
	}
...
void br_multicast_handle_new_sta(struct net_device *dev, const u8 *src)
{
	...
	hlist_for_each_entry_safe(pg, n, &port->mglist, mglist) {
		/* Perm ones should be skipped */
		if (pg->flags & MDB_PG_FLAGS_PERMANENT)
			continue;

		if (!br_port_is_accelerable_group(&pg->key.addr))
			continue;

		/* As per design, if any accelerable group found then
		 * only send a QUERY rather than sending blindly.
		 */
		send_query = true;
		break;
	}
	...
	/* Delaying explicit QUERY by 2 sec to reduce the number of
	 * duplicate QUERY on the same port on subsequent reconnect.
	 */
	if (!pmctx->ip4_sta_query.delay_time) {
		pmctx->ip4_sta_query.delay_time = 2;
		mod_timer(&pmctx->ip4_sta_query.timer, jiffies +
			  pmctx->ip4_sta_query.delay_time * HZ);
	}
#if IS_ENABLED(CONFIG_IPV6)
	if (!pmctx->ip6_sta_query.delay_time) {
		pmctx->ip6_sta_query.delay_time = 2;
		mod_timer(&pmctx->ip6_sta_query.timer, jiffies +
			  pmctx->ip6_sta_query.delay_time * HZ);
	}
#endif
...
EXPORT_SYMBOL_GPL(br_multicast_handle_new_sta);
				...
				struct net_bridge_mcast_port *pmctx)
{
	...
	timer_setup(&pmctx->ip4_sta_query.timer,
		    br_ip4_multicast_port_send_query_explicit, 0);
#if IS_ENABLED(CONFIG_IPV6)
	timer_setup(&pmctx->ip6_mc_router_timer,
		    br_ip6_multicast_router_expired, 0);
	timer_setup(&pmctx->ip6_own_query.timer,
		    br_ip6_multicast_port_query_expired, 0);
	timer_setup(&pmctx->ip6_sta_query.timer,
		    br_ip6_multicast_port_send_query_explicit, 0);
#endif
}
...
void br_multicast_port_ctx_deinit(struct net_bridge_mcast_port *pmctx)
{
	...
	del_timer_sync(&pmctx->ip6_sta_query.timer);
#endif
	del_timer_sync(&pmctx->ip4_mc_router_timer);
	del_timer_sync(&pmctx->ip4_sta_query.timer);
}
