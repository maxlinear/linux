...
static inline bool check_hw_macdb(unsigned char *macaddr,
				  struct net_device *br_dev)
{
#if IS_ENABLED(CONFIG_PPA)
	if (!ppa_hook_bridge_entry_hit_time_fn)
		return false;
	return (ppa_hook_bridge_entry_hit_time_fn
			(macaddr, br_dev, NULL) == PPA_HIT);
#endif /* IS_ENABLED(CONFIG_PPA) */
	return false;
}
...
void br_fdb_cleanup(struct work_struct *work)
{
	...
	hlist_for_each_entry_rcu(f, &br->fdb_list, fdb_node) {
		...
		if (time_after(this_timer, now)) {
			...
			/* Check whether mac exists in ppa macd,
			 * if exists, then there are active session HW
			 * associated with this mac, hence do not delete.
			 */
			if (!test_bit(BR_FDB_LOCAL, &f->flags) &&
			    check_hw_macdb(f->key.addr.addr, br->dev)) {
				f->updated = now;
