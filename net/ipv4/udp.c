...
extern int32_t (*ppa_sw_litepath_udp_send_hook)(struct sk_buff *skb);
#endif
...
send:
#if IS_ENABLED(CONFIG_PPA_UDP_LITEPATH)
	if (ppa_sw_litepath_udp_send_hook) {
		err = ppa_sw_litepath_udp_send_hook(skb);
		if (!err)
			goto xmit_done;
	}
#endif
...
int udpst_recvmsg(struct sock *sk, struct msghdr *msg, size_t len, int noblock,
		  int flags, int *addr_len)
{
	struct inet_sock *inet = inet_sk(sk);
	DECLARE_SOCKADDR(struct sockaddr_in *, sin, msg->msg_name);
	struct sk_buff *skb;
	unsigned int ulen, val_data, copied = 0;
	int peeking, off;
	int err;
	int is_udplite = IS_UDPLITE(sk);
...
try_again:
	...
	val_data = len;
	if (val_data > ulen - off)
		val_data = ulen - off;
	else if (val_data < ulen)
	...
	if (err < 0) {
		/* NOTE: We don't support peek here. */
		atomic_inc(&sk->sk_drops);
		UDP_INC_STATS(sock_net(sk),
			      UDP_MIB_INERRORS, is_udplite);
		kfree_skb(skb);
		goto rcv_buff_complete;
	}
