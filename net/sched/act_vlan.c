			...
			struct tcf_result *res)
{
	...
	case TCA_VLAN_ACT_MODIFY:
		...
		if (p->tcfv_push_flags & ACTVLAN_PUSH_F_PRIO) {
			tci &= ~VLAN_PRIO_MASK;
			tci |= p->tcfv_push_prio << VLAN_PRIO_SHIFT;
		}
		/* keep protocol if not specified */
		if (!(p->tcfv_push_flags & ACTVLAN_PUSH_F_PROTO))
			p->tcfv_push_proto = skb->vlan_proto;
