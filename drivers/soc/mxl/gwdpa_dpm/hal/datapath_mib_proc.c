...
struct dp_mib {
	...
	struct dp_pmac_count pmac0[MAX_DP_PORTS];
...
static int dp_read_pmac_counters(int inst, u8 pmacid, struct dp_mib *dp_count)
{
	...
	if (pmacid == 0)
		...
		pmac = dp_count->pmac1;
	else if ((pmacid == 2) && (is_soc_tpz(inst)))
		pmac = dp_count->pmac2;
	else
		return DP_FAILURE;
...
//	    (k >= MAX_DP_PORTS) && (l >= MAX_DP_PORTS))
//		return -1;
//	while ((i < cap->max_num_bridge_port) && (!bp[i].b_eg_valid))
//		i++;
//	while ((j < cap->max_num_subif) && (!ctp[j].eg_valid))
//		j++;
//	while ((k < MAX_DP_PORTS) && (!pmac0[k].eg_valid))
//		k++;
//	while ((l < MAX_DP_PORTS) && (!pmac1[l].eg_valid))
//		l++;
//	dp_sprintf(s, "%s", "|");
...
//	}
//	dp_sprintf(s, "%s", "|");
//	if ((k < MAX_DP_PORTS) && (pmac0[k].eg_valid)) {
//		dp_sprintf(s, "%4d  ", k);
//		dp_sprintf(s, "%13llu", pmac0[k].eg);
//		dp_sprintf(s, "%13llu", pmac0[k].eg_err);
//	} else {
...
//	}
//	dp_sprintf(s, "%s", "|");
//	if ((l < MAX_DP_PORTS) && (pmac1[l].eg_valid)) {
//		dp_sprintf(s, "%4d  ", l);
...
//	else
//		dp_count->p_eg_h[0] = MAX_DP_PORTS;
//	if (l < MAX_DP_PORTS)
//		dp_count->p_eg_h[1] = l + 1;
//	else
//		dp_count->p_eg_h[1] = MAX_DP_PORTS;
	return 0;
}
...
//	l = dp_count->xg_tx_h;
//	if ((i >= MAX_DP_PORTS) && (j >= MAX_DP_PORTS) &&
//	    (k >= cap->max_num_subif) && (l >= (dp_count->max_mac)))
//		return -1;
//	while ((i < MAX_DP_PORTS) && (!pmac0[i].ig_valid))
//		i++;
//	while ((j < MAX_DP_PORTS) && (!pmac1[j].ig_valid))
//		j++;
...
//		l++;
//	dp_sprintf(s, "%s", "|");
//	if ((i < MAX_DP_PORTS) && (pmac0[i].ig_valid)) {
//		dp_sprintf(s, "%4d  ", i);
...
//	}
//	dp_sprintf(s, "%s", "|");
//	if ((j < MAX_DP_PORTS) && (pmac1[j].ig_valid)) {
//		dp_sprintf(s, "%4d  ", j);
...
//	else
//		dp_count->p_ig_h[0] = MAX_DP_PORTS;
//	if (j < MAX_DP_PORTS)
//		dp_count->p_ig_h[1] = j + 1;
//	else
//		dp_count->p_ig_h[1] = MAX_DP_PORTS;
...
//	if ((i >= MAX_DP_PORTS) && (j >= MAX_DP_PORTS))
//		return -1;
//	while ((i < MAX_DP_PORTS) && (!pmac2[i].eg_valid))
//		i++;
//	while ((j < MAX_DP_PORTS) && (!pmac2[j].ig_valid))
//		j++;
//	dp_sprintf(s, "%s", "|");
//	if ((i < MAX_DP_PORTS) && (pmac2[i].eg_valid)) {
//		dp_sprintf(s, "%4d: ", i);
...
//	}
//	dp_sprintf(s, "%s", "|");
//	if ((j < MAX_DP_PORTS) && (pmac2[j].ig_valid)) {
//		dp_sprintf(s, "%4d: ", j);
...
//	else
//		dp_count->p_eg_h[MAX_GSWIP_PMAC - 1] = MAX_DP_PORTS;
//	if (j < MAX_DP_PORTS)
//		dp_count->p_ig_h[MAX_GSWIP_PMAC - 1] = j + 1;
//	else
//		dp_count->p_ig_h[MAX_GSWIP_PMAC - 1] = MAX_DP_PORTS;
	return 0;
}
...
//		break;
//	case G_PMAC:
//		print_pmac_info(s, dp_count, MAX_DP_PORTS);
//		break;
//	default:
