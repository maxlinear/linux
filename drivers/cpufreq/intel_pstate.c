...
struct pstate_funcs {
	...
	void (*cpuidle_sync)(struct cpudata *cpu, int pstate);
};

static int lgm_mod_pstate[4]; /* nr cpus max */
...
static unsigned int max_freq __read_mostly;
static unsigned int init_freq __read_mostly;
static int pstate_threshold __read_mostly = 20;
static int cstate_margin __read_mostly = 1;
static volatile u64 last_pstate_time;
static unsigned int pstate_i2c_interval __read_mostly = 10;
static DEFINE_RAW_SPINLOCK(pstate_i2c_lock);
...
/*  [cpu][current pstate][new pstate requested]*/
static unsigned long pstate_stat[4][PSTATE_MAX][PSTATE_MAX];

static ssize_t write_pstate_trans(struct file *file, const char __user *ubuf,
				  size_t len, loff_t *offp)
{
	mutex_lock(&intel_pstate_driver_lock);
	memset(pstate_stat, 0, sizeof(pstate_stat));
	mutex_unlock(&intel_pstate_driver_lock);

	return len;
}

static int show_pstate_trans(struct seq_file *s, void *v)
{
	int i, j, k;

	for (k = 0; k < 4; k++) {
		seq_printf(s, "############  CPU%d ##############\n\n", k);
		seq_puts(s, "x-axis --> requested pstate)\n");
		seq_puts(s, "y-axis --> current pstate\n");
		seq_puts(s, "Socwatch: P24:624, P22:780, P20:936, P18:1092, P16:1248, P14:1404, P12:1506, P10:1716, P8 :1872, P6 :2028\n");
		seq_puts(s, "MXL     : P8 :624, P10:780, P12:936, P14:1092, P16:1248, P18:1404, P20:1506, P22:1716, P24:1872, P26:2028\n");
		seq_puts(s, "    | ");
		for (i = 0; i < PSTATE_MAX; i++)
			seq_printf(s, "  P%2d |", i);
		seq_puts(s, "\n");
		mutex_lock(&intel_pstate_driver_lock);
		for (i = 0; i < PSTATE_MAX; i++) {
			seq_printf(s, " P%2d| ", i);
			for (j = 0; j < PSTATE_MAX; j++)
				seq_printf(s, "%5ld |", pstate_stat[k][i][j]);
			seq_puts(s, "\n");
		}
		seq_puts(s, "\n");
		mutex_unlock(&intel_pstate_driver_lock);
...
static int open_pstate_trans(struct inode *inode, struct file *file)
{
	return single_open(file, show_pstate_trans, NULL);
}

static const struct file_operations pstate_trans_fops = {
	.open		= open_pstate_trans,
	.write		= write_pstate_trans,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
/* Added parameter 'not_used' as part of v5.15.94 upgrade */
static int lgm_get_max_pstate(int not_used)
{
	...
	rdmsrl(MSR_IA32_PLATFORM_ID, value);
	return (value >> 8) & 0xff;
#endif
	/* Currently, the max pstat is not supported,
	 * Set the max value to 26 as the temp max pstate
	 */
	if (max_freq)
		freq = clamp_t(unsigned int, max_freq / 78000, 0x8, 0x1a);
...
#define pstate_threshold_0	20

static void lgm_cpuidle_sync(struct cpudata *cpu, int pstate)
{
	const int index[4] = {1, 0, 3, 2};
	const int pair = index[cpu->cpu & 0x3];

	if ((pstate > pstate_threshold) &&
	    (READ_ONCE(lgm_mod_pstate[cpu->cpu]) != 1)) {
		WRITE_ONCE(lgm_mod_pstate[cpu->cpu], 1);

		intel_idle_disable_cstate(cpu->cpu, 1, cstate_margin);
		intel_idle_disable_cstate(pair, 1, cstate_margin);
	} else if ((pstate <= pstate_threshold) &&
		   (READ_ONCE(lgm_mod_pstate[cpu->cpu]) == 1)) {
		WRITE_ONCE(lgm_mod_pstate[cpu->cpu], 0);

		if (READ_ONCE(lgm_mod_pstate[pair]) == 0) {
			intel_idle_disable_cstate(cpu->cpu, 0, 0);
			intel_idle_disable_cstate(pair, 0, 0);
		}
	}
}

#if IS_ENABLED(CONFIG_PSTATE_DEBUG)
static int intel_pstate_debug_adjust_pstate(struct cpudata *cpu, int pstate)
{
	pstate = cpu->pstate.current_pstate + 2;

	if (pstate > cpu->max_pstate)
		pstate = cpu->min_pstate;
...
static void intel_pstate_update_pstate(struct cpudata *cpu, int pstate)
{
	...
	pstate = intel_pstate_debug_adjust_pstate(cpu, pstate);

	if (pstate == cpu->pstate.current_pstate)
		return;

	/* If current CPU frequency is greater than or equal to 1.5G,
	 * then CPU frequency will be increased step by step.
	 * If current CPU frequency is less than 1.5G,
	 * then CPU frequency can only be increased to 1.5G at most.
	 */
	if (pstate > pstate_threshold_0 &&
	    pstate > cpu->pstate.current_pstate) {
		if (cpu->pstate.current_pstate < pstate_threshold_0)
			pstate = pstate_threshold_0;
		else
			pstate = cpu->pstate.current_pstate + 2;
	}

	/* If Current CPU frequency is greater than 1.5G,
	 * then CPU frequency will be decreased step by step
	 */
	if (cpu->pstate.current_pstate > pstate_threshold_0 &&
	    pstate < cpu->pstate.current_pstate)
		pstate = cpu->pstate.current_pstate - 2;

	if (pstate_funcs.cpuidle_sync && pstate > pstate_threshold)
		pstate_funcs.cpuidle_sync(cpu, pstate);

#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* pstate transition statistic field */
	trans = pstate_stat[cpu->cpu][cpu->pstate.current_pstate][pstate];
	trans++;
	pstate_stat[cpu->cpu][cpu->pstate.current_pstate][pstate] = trans;
#endif

	cpu->pstate.current_pstate = pstate & 0xfffffffe;
				     ...
				     unsigned int flags)
{
	...
	if ((s64)delta_ns < INTEL_PSTATE_SAMPLING_INTERVAL)
		return;
#endif
	raw_spin_lock_irqsave(&pstate_i2c_lock, irq_flags);
	if (ktime_to_ns(ktime_sub(ktime_get(),  READ_ONCE(last_pstate_time)))
	    < (pstate_i2c_interval * NSEC_PER_MSEC)) {
		raw_spin_unlock_irqrestore(&pstate_i2c_lock, irq_flags);
		return;
	}
	WRITE_ONCE(last_pstate_time, ktime_get());
	raw_spin_unlock_irqrestore(&pstate_i2c_lock, irq_flags);
...
static const struct x86_cpu_id intel_pstate_cpu_ids[] = {
	...
	X86_MATCH(ATOM_AIRMONT_NP,	lgm_funcs),
...
static unsigned int intel_cpufreq_get(unsigned int cpunum)
{
	struct cpudata *cpu = all_cpu_data[cpunum];

	return cpu->pstate.current_pstate * cpu->pstate.scaling;
}
...
static int intel_pstate_set_policy(struct cpufreq_policy *policy)
{
	...
	if (cpu->policy == CPUFREQ_POLICY_PERFORMANCE) {
		...
		if (pstate_funcs.cpuidle_sync &&
		    cpu->pstate.max_pstate > pstate_threshold)
			pstate_funcs.cpuidle_sync(cpu, cpu->pstate.max_pstate);
		...
		if (target_pstate != cpu->pstate.current_pstate) {
			if (pstate_funcs.cpuidle_sync &&
			    target_pstate > pstate_threshold)
				pstate_funcs.cpuidle_sync(cpu, target_pstate);
...
static int __intel_pstate_cpu_init(struct cpufreq_policy *policy)
{
	...
	unsigned int state_count, i, step = 1;
	u64 val;
	...
	state_count = (global.no_turbo || global.turbo_disabled ?
		      cpu->pstate.max_pstate : cpu->pstate.turbo_pstate) -
		      cpu->pstate.min_pstate + 1;
	if (boot_cpu_data.x86_model == INTEL_FAM6_ATOM_AIRMONT_NP) {
		state_count = (state_count + 1) >> 1;
		step = 2;
	}

	freq_table = kcalloc(state_count + 1, sizeof(*freq_table), GFP_KERNEL);
	if (!freq_table)
		return -ENOMEM;
	...
	for (i = 0; i < state_count; i++) {
		...
		freq_table[i].frequency = pstate * cpu->pstate.scaling;
		pstate += step;
	}
	freq_table[i].frequency = CPUFREQ_TABLE_END;
	policy->freq_table = freq_table;

	rdmsrl_on_cpu(cpu->cpu, MSR_IA32_MISC_ENABLE, &val);
	if (!(val & MSR_IA32_MISC_ENABLE_ENHANCED_SPEEDSTEP)) {
		val |= MSR_IA32_MISC_ENABLE_ENHANCED_SPEEDSTEP;
		wrmsrl_on_cpu(cpu->cpu, MSR_IA32_MISC_ENABLE, val);
		/* Can be locked at 0 */
		rdmsrl_on_cpu(cpu->cpu, MSR_IA32_MISC_ENABLE, &val);
		if (!(val & MSR_IA32_MISC_ENABLE_ENHANCED_SPEEDSTEP)) {
	...
	if (init_freq)
		intel_cpufreq_target(policy, init_freq, CPUFREQ_RELATION_L);
...
static struct cpufreq_driver intel_pstate = {
	.flags		= CPUFREQ_CONST_LOOPS | CPUFREQ_IS_COOLING_DEV,
	.verify		= intel_pstate_verify_policy,
	.setpolicy	= intel_pstate_set_policy,
	.suspend	= intel_pstate_suspend,
	.resume		= intel_pstate_resume,
	.init		= intel_pstate_cpu_init,
	.get		= intel_cpufreq_get,
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
		/* pstate transition statistic field */
		trans = pstate_stat[cpu->cpu][old_pstate][target_pstate];
		trans++;
		pstate_stat[cpu->cpu][old_pstate][target_pstate] = trans;
#endif
...
static void __init copy_cpu_funcs(struct pstate_funcs *funcs)
{
	...
	pstate_funcs.cpuidle_sync   = funcs->cpuidle_sync;
}
...
static int __init intel_max_cpufreq_setup(char *str)
{
	int freq;
...
early_param("max_cpufreq", intel_max_cpufreq_setup);

static int __init intel_init_cpufreq_setup(char *str)
{
	int freq;
...
early_param("init_cpufreq", intel_init_cpufreq_setup);

static int __init init_pstate_threshold(char *str)
{
	int max_p;

	if (get_option(&str, &max_p))
		pstate_threshold = max_p;

	return 0;
}
early_param("init_pstate_threshold", init_pstate_threshold);

static int __init init_cstate_margin(char *str)
{
	int max_c;

	if (get_option(&str, &max_c))
		cstate_margin = max_c;

	return 0;
}
early_param("init_cstate_margin", init_cstate_margin);

static int __init init_pstate_i2c_interval(char *str)
{
	int interval;

	if (get_option(&str, &interval))
		pstate_i2c_interval = interval;

	return 0;
}
early_param("pstate_i2c_interval", init_pstate_i2c_interval);
