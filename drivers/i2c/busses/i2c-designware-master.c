...
static void i2c_poll_tx_fifo(struct dw_i2c_dev *dev)
{
	u32 val;
	int ret;

	ret = regmap_read_poll_timeout(dev->map, DW_IC_STATUS, val,
				       val & DW_IC_STATUS_TFE,
				       200, 20000);
	if (ret)
		dev_warn(dev->dev, "timeout in cleaning tx fifo\n");
}
...
i2c_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if (dev->flags & MODEL_MXL_LGM) {
		/* make sure i2c controller is really disabled. */
		__i2c_dw_disable(dev);
