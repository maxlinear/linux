 ...
 * Hardware transmit 32-bit data in big-endian order, but the start-point
 * depends on number of bytes.
 */
static u32 lantiq_ssc_tx_word(struct lantiq_ssc_spi *spi, u32 num_of_bytes)
{
	const u8 *tx = spi->tx;
	u32 data = 0;
	int i;
...
/**
 * Transform bytes to ssc word.
 * Hardware fills 32-bit data in big-endian order, but the start-point
 * depends on number of bytes.
 */
static void lantiq_ssc_rx_word(struct lantiq_ssc_spi *spi, u32 num_of_bytes,
			       u32 data)
{
	u8 *rx = spi->rx;
	int i;
...
static void tx_fifo_write(struct lantiq_ssc_spi *spi)
{
	...
	while (spi->tx_todo && tx_free) {
		...
		data = lantiq_ssc_tx_word(spi, bytes);
...
static void rx_fifo_read_full_duplex(struct lantiq_ssc_spi *spi)
{
	...
	while (rx_fill) {
		...
		lantiq_ssc_rx_word(spi, bytes, data);
		rx_fill--;
	}
}
...
static void rx_fifo_read_half_duplex(struct lantiq_ssc_spi *spi)
{
	...
	while (rx_fill) {
		if (spi->rx_todo < 4)
			bytes = (lantiq_ssc_readl(spi, LTQ_SPI_STAT) &
