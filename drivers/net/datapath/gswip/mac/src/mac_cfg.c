...
int mac_get_stat_flowctrl(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u32 flow_ctrl_tx, flow_ctrl_rx, flow_ctrl = GSW_FLOW_AUTO;
	...
	flow_ctrl_tx = gswss_get_stat_flowctrl_tx(pdev);
	flow_ctrl_rx = gswss_get_stat_flowctrl_rx(pdev);
...
int mac_reset(void *pdev, u32 reset)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u32 off[17] = {
		MDIO_SCCDR, MAC_EXTCFG,
		MAC_TX_FCR, MAC_RX_FCR, MAC_PMT_CSR,
		MAC_LPI_TCR, MAC_LPI_CSR,
		MAC_MACA0HR, MAC_MACA0LR,
		MAC_SUBSEC_INCR, MAC_TSTAMP_ADDNDR, MAC_SYS_TIME_HWS,
		MAC_AUX_CTRL, MAC_TSTAMP_CR,
		MAC_PKT_FR, MAC_TX_CFG, MAC_RX_CFG
	};
	u32 val[17] = {0};
	size_t tscr_idx = 0, i;
	...
	if (reset == 2) {
		for (i = 0; i < ARRAY_SIZE(off); i++) {
			if (off[i] == MAC_TSTAMP_CR)
				tscr_idx = i;
			val[i] = XGMAC_RGRD(pdata, off[i]);
		}
		val[0] &= 0x80380000;
	}
	...
	if (reset == 0 || reset == 2) {
		gswss_xgmac_reset(pdev, 0);
		gswss_lmac_reset(pdev, 0);
		//gswss_set_xgmac_rx_disable(pdev, 0);
		//gswss_set_xgmac_tx_disable(pdev, 0);
	}

	if (reset == 2) {
		if (pdata->systime_initialized
		    && (val[tscr_idx] & BIT(MAC_TSTAMP_CR_TSENA_POS))) {
			struct timespec64 now;

			XGMAC_RGWR_BITS(pdata, MAC_TSTAMP_CR, TSENA, 1);
			ktime_get_real_ts64(&now);
			xgmac_init_systime(pdev, now.tv_sec, now.tv_nsec);

			val[tscr_idx] |= BIT(MAC_TSTAMP_CR_TSADDREG_POS);
		}

		for (i = 0; i < ARRAY_SIZE(off); i++)
			XGMAC_RGWR(pdata, off[i], val[i]);
	}
...
int mac_get_stat_duplex(void *pdev)
{
	int val = 0;
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	...
	val = gswss_get_stat_duplex_mode(pdev);
...
int mac_set_speed(void *pdev, u32 phy_speed)
{
	...
	} else if (phy_speed == GSW_PORT_SPEED_2500) { /* 2.5Gbps */
	...
	} else if (phy_speed == GSW_PORT_SPEED_10000) {  /* 10Gbps */
...
int mac_get_speed(void *pdev)
{
	...
	if (mac_speed == SPEED_10M)
		...
		mac_speed = GSW_PORT_SPEED_10000;
	else if (mac_speed == SPEED_2G5)
		mac_speed = GSW_PORT_SPEED_2500;
	else if (mac_speed == SPEED_AUTO)
		mac_speed = 0;
...
int mac_get_stat_speed(void *pdev)
{
	int mac_speed;
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	...
	mac_speed = gswss_get_stat_speed(pdev);
	if (mac_speed == SPEED_10M)
		mac_speed = GSW_PORT_SPEED_10;
	else if (mac_speed == SPEED_100M)
		mac_speed = GSW_PORT_SPEED_100;
...
int mac_get_stat_linksts(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	int linksts;
	...
	linksts = gswss_get_stat_linkstatus(pdev);
...
int mac_set_lpien(void *pdev, u32 enable, u32 lpi_waitg, u32 lpi_waitm)
{
...
#ifndef CONFIG_SILVER_WORKAROUND
	...
	xgmac_set_eee_timer(pdev, pdata->twt, pdata->lst,
			    max(lpi_waitg, lpi_waitm));
	gswss_set_eee_cap(pdev, enable ? EEE_CAP_AUTO : EEE_CAP_OFF);
#endif
...
int mac_get_lpien(void *pdev)
{
	...
	enable = gswss_get_eee_cap(pdev) == EEE_CAP_OFF ? 0 : 1;
...
int mac_enable_ts(void *pdev)
{
	...
	/* Enable the Ingress Special Tag processing at PCE  */
...
int mac_get_int_sts(void *pdev)
{
	...
	if (mac_isr) {
		...
		if (MAC_GET_VAL(mac_isr, MAC_ISR, LSI)) {
			val |= (1 << XGMAC_LSI_EVNT);
			pdata->xgmac_ls = MAC_GET_VAL(mac_isr, MAC_ISR, LS);
		}
	}
...
int mac_init(void *pdev)
{
	...
	mac_set_mii_if(pdev, GSW_PORT_HW_XGMII);
	if (!pdata->usxgmii)
		gswss_set_2G5_intf(pdev, XGMAC_GMII);
	else
		gswss_set_fe_intf(pdev, XGMAC_GMII);
...
#endif
	xgmac_forward_fup_fep_pkt(pdev,
				  pdata->xgmac_err_pkt_fwd_fup,
				  pdata->xgmac_err_pkt_fwd_fef);
#ifdef __KERNEL__

	/* Configure MAC Speed to 10/2.5/1 G */
	if (pdata->haps)
		mac_set_speed(pdev, pdata->phy_speed);
	else
	...
	gswss = dev_get_drvdata(pdata->dev->parent);
	if (!gswss || !gswss->clk)
		ticcnt = 399;
	else {
		ticcnt = (u32)(clk_get_rate(gswss->clk) / 2000000);
		if (ticcnt > 0)
			ticcnt--;
	}
	xgmac_wr_reg(pdev, MAC_1US_TIC_CNT, ticcnt);

	/* Tell adaption layer to remove FCS in Rx Direction */
	gswss_set_mac_rxfcs_op(pdev, MODE3);

	/* Insert Dummy Special tag from GSWIP Subsys if
	 * CONFIG_MXL_ETHSW_NOSPTAG is set.
	 *
	 * The adaption layer between MAC and GSWIP could hang if a special
	 * tag is added especially in half duplex mode.
	 * The PCE rules are either matching for packets with or without
	 * special tag, but not for both types. The PON interfaces always have
	 * special tags. If LAN ports do not have a special tag additional
	 * rules are needed for common PCE rules.
	 */
#if defined(CONFIG_MXL_ETHSW_NOSPTAG)
	gswss_set_mac_rxsptag_op(pdev, (RX_SPTAG_NOTAG % 4));
#else
	gswss_set_mac_rxsptag_op(pdev, (RX_SPTAG_INSERT % 4));
#endif

	/* By default setting FDMA Don't Remove FCS @ Tx
	 * PMAC is now set as no dummy FCS addition @ Tx (in pmac_glbl_cfg),
...
#endif
	xgmac_forward_fup_fep_pkt(pdev, 1, 0);

	/* Special tag configuration if external switch is connected.
	 * For example, GSW120 connected to PRX300 */
	if (pdata->tx_sptag)
...
int mac_int_enable(void *pdev)
{
	...
	/* Enable LMAC Interrupt */
	lmac_set_int(pdev, 1);
	/* Enable XGMAC Interrupt */
	gswss_set_interrupt(adap_ops, XGMAC, pdata->mac_idx, 1);
	/* Enable LINK Interrupt */
	gswss_set_interrupt(adap_ops, LINK, pdata->mac_idx, 1);
...
int mac_int_disable(void *pdev)
{
	...
	/* Disable LMAC Interrupt */
	lmac_set_int(pdev, 0);
	/* Disable XGMAC Interrupt */
	gswss_set_interrupt(adap_ops, XGMAC, pdata->mac_idx, 0);
	/* Disable LINK Interrupt */
	gswss_set_interrupt(adap_ops, LINK, pdata->mac_idx, 0);
...
static void xgmac_lsi_isr_hdlr(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);

	mac_dbg("MAC %d: Recieved LSI Interrupt %d\n", pdata->mac_idx,
		pdata->xgmac_ls);
	switch (pdata->xgmac_ls) {
	case XGMAC_LINK_OK:
		mac_set_linksts(pdev, GSW_PORT_LINK_AUTO);
		break;
	case XGMAC_LOCAL_LINK_FAULT:
		mac_set_linksts(pdev, GSW_PORT_LINK_DOWN);
		break;
	default:
		break;
	}
...
{
	struct macsec_pvt_data *pdata = platform_get_drvdata(pdev);
...
static int mac_offload_mac_check(void *pdev, struct mac_eip160_ops *e160)
{
	int ret = 0;
	...
	if (pdata->mac_idx < MAC_2 || pdata->mac_idx > MAC_4) {
		pr_info("MACSEC not support offload mac");
		ret = -EINVAL;
	}
	return ret;
}

int mac_eip160_prepare(void *pdev, struct mac_eip160_ops *e160)
{
	int ret = 0;
	struct mac_prv_data *pdata;
	struct device *dev;
	...
	if (pdata->ig_pdev && pdata->eg_pdev)
		return 0;
...
int mac_eip160_unprepare(void *pdev, struct mac_eip160_ops *e160)
{
	int mac_idx;
	struct device *dev;
	struct gswss *gswdev;
	...
	mac_idx = ops->ss_get_macsec_mac(ops);
	/* Disable MACsec to MAC map */
	if (pdata->ig_pdev && pdata->eg_pdev)
		ops->ss_set_macsec_mac(ops, mac_idx, 0);
	/* HW reset MACsec module */
	ops->ss_reset_macsec(ops);

	e160->ig_pdev = pdata->ig_pdev = NULL;
	e160->eg_pdev = pdata->eg_pdev = NULL;
	e160->reg_rd = NULL;
	e160->reg_wr = NULL;
...
int mac_config_mode(void *pdev, phy_interface_t mode)
{
	...
	if (pdata->mode != mode) {
		pdata->mode = mode;
		if (pdata->mode == PHY_INTERFACE_MODE_USXGMII) {
			ret = gswss_set_2G5_intf(pdev, XGMAC_XGMII);
			ret |= gswss_set_fe_intf(pdev, XGMAC_GMII);
		} else {
			ret = gswss_set_2G5_intf(pdev, XGMAC_GMII);
			ret |= gswss_set_fe_intf(pdev, XGMAC_GMII);
		}
	}
...
void mac_init_fn_ptrs(struct mac_ops *mac_op)
{
	...
	mac_op->get_stat_link_sts = mac_get_stat_linksts;
	...
	mac_op->mac_offload_mac_check = mac_offload_mac_check;
	mac_op->mac_eip160_prepare = mac_eip160_prepare;
	mac_op->mac_eip160_unprepare = mac_eip160_unprepare;
	mac_op->mac_config_mode = mac_config_mode;
	mac_op->mac_receiver_enable = xgmac_receiver_enable;
}
