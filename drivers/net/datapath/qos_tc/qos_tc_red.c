...
static int wred_params_set(struct qos_tc_qdisc *sch,
			   ...
			   struct tc_red_qopt_offload_params *params)
{
	...
	switch (type) {
	case QOS_TC_QDATA_GREEN:
		id = 0;
		break;
	case QOS_TC_QDATA_YELLOW:
		id = 1;
		break;
	default:
...
{
	return __wred_destroy(dev, handle, parent, QOS_TC_QDATA_YELLOW);
}

static int wred_green_destroy(struct net_device *dev, u32 handle, u32 parent)
{
	return __wred_destroy(dev, handle, parent, QOS_TC_QDATA_GREEN);
}

static int qos_tc_red_replace(struct net_device *dev,
			      struct tc_red_qopt_offload *opt)
{
	...
	if (qos_tc_qdata_get_by_type(dev, qid, QOS_TC_QDATA_GREEN,
				     &tmp_h, &tmp_p)) {
		destroy = wred_green_destroy;
		type = QOS_TC_QDATA_GREEN;
		netdev_dbg(dev, "tc-red qdisc configuring green\n");
	} else {
		destroy = wred_yellow_destroy;
		type = QOS_TC_QDATA_YELLOW;
	...
	trace_qos_tc_red_replace(dev, opt, qdisc, qid, type);

	return 0;
}

static int qos_tc_red_destroy(struct net_device *dev,
			      struct tc_red_qopt_offload *opt)
{
	struct qos_tc_port *port = NULL;
	struct qos_tc_qdisc *qdisc = NULL;
	...
	ret = qos_tc_qdata_remove(dev, qid, opt->handle, opt->parent);

	trace_qos_tc_red_destroy(dev, opt, qdisc, qid);
		       ...
		       void *type_data)
{
	...
	trace_qos_tc_red_enter(dev, opt);

	switch (opt->command) {
	case TC_RED_REPLACE:
		netdev_dbg(dev, "replace pid:%#x class/handle:%#x\n",
			   opt->parent, opt->handle);
		err = qos_tc_red_replace(dev, opt);
		if (err < 0) {
	...
	case TC_RED_DESTROY:
		netdev_dbg(dev, "destroy pid:%#x class/handle:%#x\n",
			   opt->parent, opt->handle);
		err = qos_tc_red_destroy(dev, opt);
		if (err < 0) {
