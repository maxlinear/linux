 ...
 * ToDo: PONRTSYS-9607 - Read DP capabilities to get platform specific
 * values instead of using fix defines. To be done when dp_get_cap can
 * be used by builtin kernel modules.
 */
#define MAX_PORT 16
#define MAX_SUBIF 128
...
static s16 get_bridge_port_id(struct net_device *dev)
{
	...
	if (!subif) {
		netdev_err(dev, "Failed to allocate memory for subif\n");
		return -1;
	}

	ret = dp_get_netif_subifid(dev, NULL, NULL, NULL, subif, 0);
	if (ret != DP_SUCCESS) {
		netdev_err(dev, "Failed to get subif\n");
		ret = -1;
	} else {
...
static void update_dev_counters(struct mib_counters *cnt, u8 initial_update)
{
	...
	} else if (cnt->dev_is_vuni) {
		/* For VUNI device the TX Bypass counters are always 0.
		 * Outgoing counters can only be fetched from corresponding interface
		 * e.g. pmapper.
		 * Hence, CTP TX counter have to be reported instead.
		 * No action required!
		 */
	} else {
			   ...
			   s32 port_id, s32 sub_if, s32 inst)
{
	...
	/* check for VUNI */
	dev_cnt->dev_is_vuni = port_prop.alloc_flags & DP_F_VUNI ? 1 : 0;
						...
						struct net_device *dev)
{
	...
	lockdep_assert_held_read(&dev_list_lock);
	list_for_each(p, &registered_devices_list)
	{
		mib_cnt = list_entry(p, struct mib_counters, dev_list);
		if ((mib_cnt->port_id == port_id) &&
