...
static int directpath = 1;
module_param(directpath, int, 0644);
MODULE_PARM_DESC(directpath, "Override default directpath");

/*-------------------------------------------------------------------------*/

#if IS_ENABLED(CONFIG_DP_DIRECTPATH)
static int usbnet_directpath_rx_stop(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);

	usbnet_pause_rx(dev);
	netif_dbg(dev, rx_status, dev->net, "directpath rx: stop\n");
	return 0;
}

static int usbnet_directpath_rx_restart(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);

	usbnet_resume_rx(dev);
	netif_dbg(dev, rx_status, dev->net, "directpath rx: restart\n");
	return 0;
}

static struct dp_cb dp_ops = {
	/* note: use default rx_fn handler */
	.stop_fn = usbnet_directpath_rx_stop,
	.restart_fn = usbnet_directpath_rx_restart,
};
#endif /* CONFIG_DP_DIRECTPATH */

static void usbnet_directpath_attach(struct usbnet *dev)
{
#if IS_ENABLED(CONFIG_DP_DIRECTPATH)
	dev->dp_handle = kzalloc(sizeof(*dev->dp_handle), GFP_KERNEL);
	if (!dev->dp_handle)
		return;

	if (dpdp_register_dev_hook)
		dpdp_register_dev_hook(THIS_MODULE, dev->net,
				       dev->dp_handle,
				       &dp_ops, 1, 0);
	if (dpdp_register_subif_hook)
		dpdp_register_subif_hook(THIS_MODULE, dev->net,
					 dev->dp_handle, 0);
#endif /* CONFIG_DP_DIRECTPATH */
}

static void usbnet_directpath_detach(struct usbnet *dev)
{
#if IS_ENABLED(CONFIG_DP_DIRECTPATH)
	if (!dev->dp_handle)
		return;
	...
	if (dpdp_register_subif_hook)
		dpdp_register_subif_hook(THIS_MODULE, dev->net, dev->dp_handle,
					 DP_F_DEREGISTER);
	if (dpdp_register_dev_hook)
		dpdp_register_dev_hook(THIS_MODULE, dev->net,
				       dev->dp_handle, NULL, 1,
				       DP_F_DEREGISTER);
	kfree(dev->dp_handle);
	dev->dp_handle = NULL;
#endif /* CONFIG_DP_DIRECTPATH */
...
static ssize_t directpath_show(struct device *device,
			       struct device_attribute *attr, char *buf)
{
	struct net_device *net = to_net_dev(device);
	struct usbnet     *dev = netdev_priv(net);

	if (dev->dp_handle)
		return sprintf(buf, "%d:%d\n", dev->dp_handle->port_id,
			       dev->dp_handle->subif);
	else
		return sprintf(buf, "-1:-1\n");
}

static ssize_t directpath_store(struct device *device,
				...
				const char *buf, size_t count)
{
	...
	status = kstrtoul(buf, 0, &trigger);
	if (status < 0)
		return count;

	if (dev->dp_handle)
		usbnet_directpath_detach(dev);
	if (trigger)
		usbnet_directpath_attach(dev);

	return count;
}

static DEVICE_ATTR_RW(directpath);
static int directpath_control_create(struct device *device)
{
	return device_create_file(device, &dev_attr_directpath);
}

static void directpath_control_remove(struct device *device)
{
	device_remove_file(device, &dev_attr_directpath);
}
...
void usbnet_skb_return (struct usbnet *dev, struct sk_buff *skb)
{
	...
	if (dpdp_fwd_offload_hook) {
		...
		status = dpdp_fwd_offload_hook(dev->net, dev->dp_handle,
					       skb, 0);
		if (status == RX_HANDLER_CONSUMED) {
			netif_dbg(dev, rx_status, dev->net,
				  "dpdp_fwd_offload status %d\n", status);
			return;
		}
				     ...
				     struct net_device *net)
{
	...
	if (dpdp_qos_offload_hook) {
		retval = dpdp_qos_offload_hook(net, dev->dp_handle, skb, 0);
		if (retval != NETDEV_TX_CONTINUE) {
			netif_dbg(dev, tx_queued, dev->net,
				  "dpdp_qos_offload status %d\n", retval);
			return retval;
		}
	}
#endif /* CONFIG_DP_DIRECTPATH */
...
void usbnet_disconnect (struct usb_interface *intf)
{
	...
	net = dev->net;
	directpath_control_remove(&net->dev);
	usbnet_directpath_detach(dev);
...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	if (directpath)
		usbnet_directpath_attach(dev);
	directpath_control_create(&net->dev);
