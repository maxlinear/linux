...
s32 qos_queues_stats_bytes_show(char *buf, size_t sz, size_t *n, void *stats,
				u32 num_stats, void *data)
{
	const struct queue_stats *it;
	const struct qos_node    *node;
	...
	num  = min(qdev->init_params.max_queues, num_stats);
	for_each_arr_entry(it, stats, num) {
		if (!it->wred.total_bytes_accepted &&
			!it->wred.total_bytes_dropped)
			continue;
...
static void __queue_stats_bps_diff(const struct queue_stats *pre,
			       ...
			       struct queue_stats *delta)
{
	...
	delta->wred.total_bytes_dropped = post->wred.total_bytes_dropped -
		pre->wred.total_bytes_dropped;
	delta->wred.total_bytes_accepted = post->wred.total_bytes_accepted -
		pre->wred.total_bytes_accepted;
	delta->wred.total_bytes_dropped =
		post->wred.total_bytes_dropped - pre->wred.total_bytes_dropped;
	delta->total_fwd_bytes = post->total_fwd_bytes - pre->total_fwd_bytes;
}

s32 qos_queues_stats_bps_diff(void *pre, u32 num_pre, void *post, u32 num_post,
		void *delta, u32 num_delta, void *data)
{
	...
	post_it  = post;
	delta_it = delta;
	num      = min(qdev->init_params.max_queues, num_pre);

	for_each_arr_entry(pre_it, pre, num, post_it++, delta_it++)
		__queue_stats_bps_diff(pre_it, post_it, delta_it);
...
s32 qos_queues_stats_get(void *stats, u32 num_stats, void *data)
{
	const struct qos_node *node;
	struct pp_qos_dev     *qdev = data;
	struct queue_stats    *it = (struct queue_stats *)stats;
	u32 i, node_id, node_phy, cnt = 0;
	s32 ret;

	if (unlikely(ptr_is_null(stats) || ptr_is_null(data)))
		return -EINVAL;

	for (i = 0; i < ARRAY_SIZE(qdev->nodes->nodes); i++) {
		node = &qdev->nodes->nodes[i];
...
s32 node_remove(struct pp_qos_dev *qdev, struct qos_node *node)
{
	...
	if (node_queue(node)) {
		...
		if (node_sched(node) && node_rep(node) &&
		    node->shared_bw_group != NO_SHARED_BW_GRP) {
			s_bw_group = node->shared_bw_group;
			node->shared_bw_group = NO_SHARED_BW_GRP;
			if (_qos_shared_limit_group_remove(qdev, s_bw_group)) {
				QOS_LOG_ERR("Failed to remove shared bwl group %u\n",
...
#define post_order_travers_tree(q, r, c, cd, o, od)                            \
	travers_tree(q, r, c, cd, o, od, POST_ORDER)

#define right_to_left_travers_tree(q, r, c, cd, o, od)                         \
	travers_tree(q, r, c, cd, o, od, RIGHT_TO_LEFT)

/******************************************************************************/
/*                           Tree traversal                                   */
/******************************************************************************/
/**
 * travers_tree() - Traverse subtree - children first
 ...
 * @route:      if POST_ORDER travers from left children to right otherwise right
 * 		to left
 ...
 */
static int travers_tree(struct pp_qos_dev *qdev, s32 root,
			s32 (*conform)(const struct pp_qos_dev *qdev,
				       const struct qos_node *node,
				       void *cdata),
			void *cdata,
			s32 (*operation)(struct pp_qos_dev *qdev,
					 struct qos_node *node, void *odata),
			void *odata, enum tree_traversal route)
{
	u32 i ,j;
	u32 phy;
	u32 total;
	...
	if (node_parent(node)) {
		for (i = 0; i < node->parent_prop.num_of_children; ++i) {
			j = i;
			if (route == RIGHT_TO_LEFT) {
				/* Update j ind to iterate from right to left */
				j = node->parent_prop.num_of_children - i - 1;
			}

			phy = node->parent_prop.first_child_phy + j;
			...
			total += travers_tree(qdev, phy, conform, cdata,
					      operation, odata, route);
		}
	}
				  ...
				  struct qos_node *node, void *data)
{
	...
	phy = get_phy_from_node(qdev->nodes, node);
	bwl_entry_addr = (ulong)qdev->hwmem.bwl_ddr_virt +
	...
	sbwl_entry_addr = (ulong)qdev->hwmem.sbwl_ddr_virt + (phy * 4);
		...
		if (node_queue(child)) {
			...
			*queues_num = *queues_num + 1;
			...
			dr_child = get_node_from_phy(qdev->nodes,
						     dr_sched_child_phy);
			if (unlikely(!dr_child)) {
				QOS_LOG_ERR("get_node_fromW_phy(%u) NULL\n",
						dr_sched_child_phy);
				return;
			}
 ...
 * @return s32 true if the node should wsp_wrapper, false otherwise
 */
...
void update_bwl_buffer(struct pp_qos_dev *qdev, u32 phy)
{
	...
	bwl_val = *(u32 *)(qdev->hwmem.bwl_ddr_virt +
			   qdev->hwmem.bwl_sz - 4);

	sbwl_val = *(u32 *)(qdev->hwmem.sbwl_ddr_virt +
			    qdev->hwmem.sbwl_sz - 4);
}
...
{
	union wsp_queues_t *wsp_info = qdev->hwmem.wsp_queues_ddr_virt;
	struct wsp_queue_t *curr_q;
	u32 num_queues, idx, rlm;

	pp_cache_invalidate(wsp_info, qdev->hwmem.wsp_queues_sz);

	num_queues = wsp_info->prx.num_queues;
	curr_q = wsp_info->prx.queues;
	rlm = get_phy_from_id(qdev->rlm_mapping, id);

	for (idx = 0; idx < num_queues; idx++, curr_q++) {
		if (rlm == curr_q->rlm)
			return true;
	}
 ...
 * @return num of ewsp queues
 */
static u32 get_wsp_info(struct pp_qos_dev *qdev, union wsp_queues_t *wsp_data)
{
	struct wsp_container_metadata data;
	struct qos_node *node;
	u32 queues_num, phy, mx_size = QOS_ENHANCED_WSP_MAX_QUEUES_PRX;
	...
	for (phy = 0; phy < qdev->init_params.max_ports; phy++) {
		...
		if (queues_num >= mx_size)
			goto out;

		data.next = 0;
		data.info = wsp_data->prx.queues + queues_num;
...
out:
	wsp_data->prx.num_queues = queues_num;
...
{
	u32 rlm, mask, *ptr;
	union wsp_queues_t *wsp_data = qdev->hwmem.wsp_queues_ddr_virt;

	pp_cache_invalidate(wsp_data, qdev->hwmem.wsp_queues_sz);
	ptr = wsp_data->queues_bitmap_lgm;
				...
				void *data)
{
	...
	ptr = ((union wsp_queues_t *)data)->queues_bitmap_lgm;
	rlm = node->data.queue.rlm;

	/* set ON the bit in the rlm's index */
	ptr += rlm / 32;
	mask = 1 << (rlm % 32);
	*ptr |= mask;
...
{
	union wsp_queues_t *wsp_info;

	wsp_info = qdev->hwmem.wsp_queues_ddr_virt;
	memset(wsp_info, 0, qdev->hwmem.wsp_queues_sz);

	qdev->ewsp_db.num_queues = get_wsp_info(qdev, wsp_info);

	pp_cache_writeback(wsp_info, qdev->hwmem.wsp_queues_sz);

	PP_REG_WR32((ulong)&qdev->init_params.dccm_db->wsp_updated, 1);
}
...
s32 tree_remove(struct pp_qos_dev *qdev, u32 phy)
{
	...
	rc = right_to_left_travers_tree(qdev, phy, node_used_wrapper, NULL,
			    ...
			    const struct qos_node *node)
{
	...
	if (!((node->bandwidth_limit == QOS_NO_BANDWIDTH_LIMIT) ||
	      (node->bandwidth_limit <= qdev->max_bw_limit_kbps))) {
	...
	if (!((shared == NO_SHARED_BW_GRP) ||
	      (shared <= MAX_SHARED_BW_GRP && qdev->groups[shared].used))) {
		     ...
		     u64 *modified)
{
	...
	/* verify the parent is really a parent */
	if (!node_parent(parent)) {
		pr_err("parent %u(phy %u) cannot be parent\n", child->parent,
		       conf_parent_phy);
		return -EINVAL;
	}
...
static void node_common_init(struct qos_node *node)
{
	...
	node->shared_bw_group = NO_SHARED_BW_GRP;
	node->max_burst = QOS_DEFAULT_MAX_BURST;
}
...
s32 connect_sync_queue(struct pp_qos_dev *qdev, u32 syncq_phy, u32 dstq_phy)
{
	...
	struct pp_qos_sched_conf conf;
	u32 sched_id, sched_phy, new_dstq_phy;
	u64 modified;
...
{
	struct qos_node *dst_node, *sched_node, *parent, *discNode;
	u32 sched_id, sched_phy, dst_id, dst_phy = phy + 1;

	QOS_LOG_DEBUG("disconnect_sync_queue: phy %u\n", phy);
	discNode = get_node_from_phy(qdev->nodes, phy);
	dst_node = get_node_from_phy(qdev->nodes, dst_phy);
	if (unlikely(!dst_node)) {
		QOS_LOG_ERR("get_node_from_phy(%u) returned NULL\n", dst_phy);
		return QOS_INVALID_PHY;
	...
	QOS_BITS_SET(discNode->flags, NODE_FLAGS_QUEUE_NEW_SYNCQ);
 ...
 */
static s32 post_process_get_node_info(struct pp_qos_dev *qdev, u32 phy,
				      ...
				      struct hw_node_info_s *hw_node_info)
{
	...
	if (node_parent(node)) {
		...
		QOS_ASSERT(node->parent_prop.num_of_children == num,
			   ...
			   node->parent_prop.num_of_children, phy, num);
		...
		for (i = 0; i < num; ++i) {
			child_phy = i + first;
			id = get_id_from_phy(qdev->mapping, child_phy);
	...
	if (!node_port(node)) {
		...
		QOS_ASSERT(node->child_prop.bw_weight == weight,
			   ...
			   phy, node->child_prop.bw_weight, weight);
		...
		if (parent->parent_prop.arbitration ==
			...
			QOS_ASSERT(node->child_prop.bw_weight ==
				   ...
				   phy, QOS_CHILD_UNLIMITED_WEIGHT);
		...
		fill_preds(qdev->nodes, phy, preds,
...
{
	struct qos_node *node;
	struct hw_node_info_s node_info;
	s32 rc = 0;
	...
	rc = post_process_get_node_info(qdev, get_phy_from_node(qdev->nodes, node),
				    info, &node_info);
...
static void print_table_entry(union host_read_entry_t *read_entry, u32 table_id)
{
	switch (table_id) {
		case 0:
		{
			host_tscd_node_table_entry_t *node = &read_entry->node_entry;
			QOS_LOG_INFO("%-16s %d\n", "last_child", node->last_child);
			QOS_LOG_INFO("%-16s %d \n", "first_child", node->first_child);
			QOS_LOG_INFO("%-16s %d \n", "branch_table_idx", node->branch_table_idx);
			QOS_LOG_INFO("%-16s %d \n", "next_sch_wrr", node->next_sch_wrr);
			QOS_LOG_INFO("%-16s %d \n", "wsp_wrr_switch", node->wsp_wrr_switch);
			QOS_LOG_INFO("%-16s %d \n", "tc_wa", node->tc_wa);
			QOS_LOG_INFO("%-16s %d \n", "bw_limit", node->bw_limit);
			QOS_LOG_INFO("%-16s %d \n", "suspend", node->suspend);
			QOS_LOG_INFO("%-16s %d \n", "credit_toggle", node->credit_toggle);
			QOS_LOG_INFO("%-16s %d \n", "bw_limit_en", node->bw_limit_en);
			QOS_LOG_INFO("%-16s %d \n", "best_effort_en", node->best_effort_en);
			QOS_LOG_INFO("%-16s %d \n", "child_round_toggle", node->child_round_toggle);
			QOS_LOG_INFO("%-16s %d \n", "credit_avail", node->credit_avail);
			QOS_LOG_INFO("%-16s %d \n", "credit_inc", node->credit_inc);
			QOS_LOG_INFO("%-16s %d \n", "small_child_round_toggle", node->small_child_round_toggle);
			QOS_LOG_INFO("%-16s %d \n", "small_credit_toggle", node->small_credit_toggle);
			QOS_LOG_INFO("%-16s %d \n", "small_credit_avail", node->small_credit_avail);
			QOS_LOG_INFO("%-16s %d \n", "small_credit_inc", node->small_credit_inc);
		}
		break;
		case 1:
		{
			tscd_branch_table_entry_t *node = &read_entry->branch_entry;
			QOS_LOG_INFO("%-16s %d \n", "predecessor0", node->predecessor0);
			QOS_LOG_INFO("%-16s %d \n", "predecessor1", node->predecessor1);
			QOS_LOG_INFO("%-16s %d \n", "predecessor2", node->predecessor2);
			QOS_LOG_INFO("%-16s %d \n", "predecessor3", node->predecessor3);
			QOS_LOG_INFO("%-16s %d \n", "predecessor4", node->predecessor4);
			QOS_LOG_INFO("%-16s %d \n", "predecessor5", node->predecessor5);
		}
		break;
		case 2:
		{
			tscd_bwl_table_entry_t *node = &read_entry->bwl_entry;
			QOS_LOG_INFO("%-16s %d \n", "bwl_credit_avail", node->bwl_credit_avail);
			QOS_LOG_INFO("%-16s %d \n", "bwl_credit_inc", node->bwl_credit_inc);
			QOS_LOG_INFO("%-16s %d \n", "bwl_credit_max", node->bwl_credit_max);
			QOS_LOG_INFO("%-16s %d \n", "node_update_pending", node->node_update_pending);
			QOS_LOG_INFO("%-16s %d \n", "sbwl_entry_idx", node->sbwl_entry_idx);
		}
		break;
		case 5:
		{
			host_tscd_rl_map_table_entry_t *node = &read_entry->rlm_entry;
			QOS_LOG_INFO("%-16s %d \n", "credit_remainder", node->credit_remainder);
			QOS_LOG_INFO("%-16s %d \n", "lan_wa", node->lan_wa);
			QOS_LOG_INFO("%-16s %d \n", "leaf_node", node->leaf_node);
			QOS_LOG_INFO("%-16s %d \n", "ll_credit_req", node->ll_credit_req);
			QOS_LOG_INFO("%-16s %d \n", "lowlatency", node->lowlatency);
			QOS_LOG_INFO("%-16s %d \n", "pe_wa", node->pe_wa);
			QOS_LOG_INFO("%-16s %d \n", "pf_num", node->pf_num);
			QOS_LOG_INFO("%-16s %d \n", "port", node->port);
			QOS_LOG_INFO("%-16s %d \n", "push_credit_req", node->push_credit_req);
			QOS_LOG_INFO("%-16s %d \n", "res", node->res);
			QOS_LOG_INFO("%-16s %d \n", "suspend", node->suspend);
			QOS_LOG_INFO("%-16s %d \n", "tc_num", node->tc_num);
		}
		...
		case 6:
		{
			tscd_shared_bwl_table_entry_t *node = &read_entry->shared_bwl_entry;
			QOS_LOG_INFO("%-16s %d \n", "shared_bwl_credit_avail", node->shared_bwl_credit_avail);
			QOS_LOG_INFO("%-16s %d \n", "shared_bwl_credit_max", node->shared_bwl_credit_max);
		}
		break;
		default:
			QOS_LOG_ERR("table id (%d) not supported \n",table_id );
			return;
	}
}
/**
 * qos_table_entry_get() - Get queue's statistics
 * @qos_dev: handle to qos device instance obtained previously from qos_dev_init
 * @table_id:	table id
 * @phy_id:	physical id
 * @entry: pointer to struct to be filled with entry information
 *
 * Return: 0 on success
 */
s32 qos_table_entry_get(struct pp_qos_dev *qdev, u32 table_id, u32 phy_id)
{
	s32 rc = 0;
	u32 table_size;
	...
	switch (table_id) {
		case 0:
			table_size = sizeof(host_tscd_node_table_entry_t);
			break;
		case 1:
			table_size = sizeof(tscd_branch_table_entry_t);
			break;
		case 2:
			table_size = sizeof(tscd_bwl_table_entry_t);
			break;
		case 5:
			table_size = sizeof(host_tscd_rl_map_table_entry_t);
			break;
		case 6:
			table_size = sizeof(tscd_shared_bwl_table_entry_t);
			break;
		default:
			QOS_LOG_ERR("table id (%d) not supported \n",table_id );
			goto out;
	}
	create_get_table_entry_cmd(qdev, phy_id, table_id,
					&read_entry, table_size);
	update_cmd_id(&qdev->drvcmds);
	rc = transmit_cmds(qdev);
	if (rc) {
		QOS_LOG_ERR("transmit_cmds failed (%d)\n", rc);
	...
	print_table_entry(&read_entry, table_id);
...
s32 check_sync_with_fw(struct pp_qos_dev *qdev, bool dump_entries)
{
	...
	create_get_sys_info_cmd(qdev, &sys_stats, dump_entries);
	update_cmd_id(&qdev->drvcmds);
...
/*                                 SHARED_LIMIT                               */
/******************************************************************************/
s32 _qos_shared_limit_group_add(struct pp_qos_dev *qdev, u32 max_burst, u32 *id)
{
	...
	grp = qdev->groups + 1;
	for (i = 1; i <= MAX_SHARED_BW_GRP; ++grp, ++i) {
		if (!grp->used)
			break;
	}
...
s32 qos_shared_limit_group_add(struct pp_qos_dev *qdev, u32 max_burst, u32 *id)
{
	s32 rc;

	if (ptr_is_null(qdev) || ptr_is_null(id))
		return -EINVAL;
	...
	rc = _qos_shared_limit_group_add(qdev, max_burst, id);
	if (!rc) {
		update_cmd_id(&qdev->drvcmds);
		rc = transmit_cmds(qdev);
	}
...
s32 _qos_shared_limit_group_remove(struct pp_qos_dev *qdev, u32 id)
{
	u32 num;
	u16 tmp;
	...
	_qos_shared_limit_group_get_members(qdev, id, &tmp, 1, &num);
	if (num) {
		QOS_LOG_ERR("Share group %u has %u members cant remove\n",
				id, num);
		return -EBUSY;
	}
...
s32 qos_shared_limit_group_remove(struct pp_qos_dev *qdev, u32 id)
{
	s32 rc;
	...
	rc = _qos_shared_limit_group_remove(qdev, id);
	if (!rc) {
		update_cmd_id(&qdev->drvcmds);
		rc = transmit_cmds(qdev);
	}
...
s32 _qos_shared_limit_group_modify(struct pp_qos_dev *qdev, u32 id,
				   u32 max_burst)
{
	if (!QOS_BW_GRP_VALID(id) || !qdev->groups[id].used) {
		QOS_LOG_ERR("Shared group %u is not used\n", id);
...
s32 qos_shared_limit_group_modify(struct pp_qos_dev *qdev, u32 id,
				  u32 max_burst)
{
	s32 rc = 0;
	...
	rc = _qos_shared_limit_group_modify(qdev, id, max_burst);
	if (!rc) {
		update_cmd_id(&qdev->drvcmds);
		rc = transmit_cmds(qdev);
	}
...
s32 _qos_shared_limit_group_get_members(struct pp_qos_dev *qdev,
					u32 id, u16 *members,
					u32 size, u32 *members_num)
{
	...
	*members_num = 0;
	...
	if (!QOS_BW_GRP_VALID(id)) {
		QOS_LOG_ERR("Shared group %u is not valid\n", id);
		return -EINVAL;
	}

	if (!qdev->groups[id].used)
		return 0;
	...
	for (phy = 0; (phy < max_ports) && (size > 0); ++phy) {
		...
		if (node_used(node)) {
			get_bw_grp_members_under_node(qdev, id, phy, members,
						      size, &tmp);
			total += tmp;
			members += tmp;
			if (tmp < size)
...
s32 qos_shared_limit_group_get_members(struct pp_qos_dev *qdev, u32 id,
				       u16 *members, u32 size, u32 *members_num)
{
	s32 rc;

	if (ptr_is_null(qdev) || ptr_is_null(members) ||
	    ptr_is_null(members_num))
		return -EINVAL;
	...
	rc = _qos_shared_limit_group_get_members(qdev, id, members, size,
						 members_num);
out:
	QOS_UNLOCK(qdev);
	return rc;
...
s32 qos_dbg_qm_stat_show(struct pp_qos_dev *qdev, struct seq_file *s)
{
	...
	for (rlm_idx = 0; rlm_idx < num_rlms; rlm_idx += NUM_Q_ON_QUERY) {
		create_get_qm_stats_cmd(qdev, rlm_idx, qminfo);
			      ...
			      u32 size)
{
	...
	if (qdev->hwmem.dma_buff_sz < size) {
		QOS_LOG_ERR("dma buffer too small\n");
		return -ENOMEM;
	}

	QOS_LOCK(qdev);
	create_mcdma_copy_cmd(qdev, src_phy, (u32)qdev->hwmem.dma_buff_phys,
			      size);
	update_cmd_id(&qdev->drvcmds);
	ret = transmit_cmds(qdev);
	if (unlikely(ret))
		QOS_LOG_ERR("dma copy failed (%d)\n", ret);
	pp_cache_invalidate(qdev->hwmem.dma_buff_virt, size);
	/* copy to the user buffer */
	memcpy(dst, qdev->hwmem.dma_buff_virt, size);
	QOS_UNLOCK(qdev);
			    ...
			    u32 size)
{
	...
	memcpy(qdev->hwmem.dma_buff_virt, src, size);
	pp_cache_writeback(qdev->hwmem.dma_buff_virt, size);

	create_mcdma_copy_cmd(qdev, (u32)qdev->hwmem.dma_buff_phys, dst_phy,
			      size);
	update_cmd_id(&qdev->drvcmds);
	ret = transmit_cmds(qdev);
	if (unlikely(ret))
...
void qos_descriptor_pop(struct pp_qos_dev *qdev, u32 queue_id, u32 *desc)
{
	struct qos_node *node;
	u32 phy;
	s32 ret;
	...
	create_pop_desc_cmd(qdev, node->data.queue.rlm,
			    (u32)qdev->hwmem.dma_buff_phys);
	update_cmd_id(&qdev->drvcmds);
	ret = transmit_cmds(qdev);
	if (unlikely(ret)) {
		QOS_LOG_ERR("transmit_cmds failed (%d)\n", ret);
	...
	pp_cache_invalidate(qdev->hwmem.dma_buff_virt, PPV4_QOS_DESC_SIZE);
	memcpy((void *)desc, qdev->hwmem.dma_buff_virt, PPV4_QOS_DESC_SIZE);
...
static int qos_init_wred_queues(struct pp_qos_dev *qdev)
{
	...
	for (q = 0; q < qdev->init_params.max_queues; q++) {
		...
		if (pp_reg_poll(PP_QOS_WRED_MEM_ACCESS_BUSY_REG, BIT_ULL(0), 0)) {
	...
	if (IS_ENABLED(CONFIG_SOC_LGM)) {
		...
		// Wait for init done
		if (pp_reg_poll(WRED_INIT_REG, BIT_ULL(31), 1)) {
