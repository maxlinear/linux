...
s32 __must_check transmit_cmds(struct pp_qos_dev *qdev)
{
	...
	/* Now waiting on all commands to complete before continue */
	/* TBD: check which commands should wait_on_complete */
	qdev->init_params.fwcom.wait_on_complete = true;
	...
	if (!cmd_queue_is_empty(qdev->drvcmds.cmdq)) {
		if (qdev->init_params.fwcom.wait_for_answer == 1) {
			qdev->init_params.fwcom.wait_for_answer = 0;
		...
		if (qdev->init_params.fwcom.wait_on_complete) {
			qdev->init_params.fwcom.wait_on_complete = 0;
			rc = check_completion(qdev);
			if (unlikely(rc)) {
				pr_err("Failed sending command\n");
				return -EINVAL;
			...
			qdev->init_params.fwcom.wait_for_answer = 1;
		}
	}
...
s32 is_node_configured(const struct pp_qos_dev *qdev, u32 id, bool *configured)
{
	struct qos_node *node;
	u32 phy;
...
void pp_qos_port_conf_set_default(struct pp_qos_port_conf *conf)
{
	...
	conf->common_prop.shared_bw_group = NO_SHARED_BW_GRP;
	conf->common_prop.max_burst = QOS_DEFAULT_MAX_BURST;
				 ...
				 u16 *rlms, u32 queues_num)
{
	...
	for (queue_idx = 0; queue_idx < queues_num; queue_idx++) {
		...
		addr = (unsigned long)qdev->hwmem.max_allow_ddr_virt +
			(*rlms * sizeof(u32));

		*(u32 *)addr = node->data.queue.max_allowed;
		rlms++;
	}
	pp_cache_writeback(qdev->hwmem.max_allow_ddr_virt,
			   qdev->hwmem.max_allow_sz);
			    ...
			    const struct pp_qos_port_conf *conf)
{
	...
	if (modified) {
		...
		if (create_set_port_cmd(qdev, conf, phy, modified))
			return 0;
	}
...
static s32 _pp_qos_sched_set(struct pp_qos_dev *qdev, u32 id,
			     const struct pp_qos_sched_conf *conf);

static s32 _rep_sched_set(struct pp_qos_dev *qdev, u32 id,
			  struct pp_qos_sched_conf *conf)
{
	...
	/*
	 * if the original sched has bwl, they both must share their bw via
	 * shared bw group, and the replica gets its bw from the shared group
	 */
	if (conf->common_prop.shared_bw_group != NO_SHARED_BW_GRP)
		conf->common_prop.bandwidth_limit = SCHED_REP_BW;
	...
	conf->sched_child_prop.parent = get_rep_id_from_id(qdev->mapping,
						conf->sched_child_prop.parent);

	rc = _pp_qos_sched_set(qdev, rep_id, conf);
	if (rc) {
		QOS_LOG_ERR("Set sched failed\n");
		return rc;
	}
...
/**
 * _qos_port_stat_get() - Get port's statistics and translate it
 * from uc format to host format
 * @qos_dev: handle to qos device instance obtained previously from qos_dev_init
 * @id: queue's id obtained from queue_allocate
 * @port_stat: pointer to struct to be filled with queue's statistics
 * @reset_stats: reset stats after reading
 *
 * Return: 0 on success
 */
static s32 _qos_port_stat_get(struct pp_qos_dev *qdev, u32 id,
			struct pp_qos_port_stat *port_stat, bool reset_stats)
{
	struct qos_node *node;
	bool node_configured;
	struct port_stats_s uc_port_stats = {0};
	s32 rc;
	...
	create_get_port_stats_cmd(qdev, get_phy_from_node(qdev->nodes, node),
				  &uc_port_stats, reset_stats);
	...
	port_stat->debug_actual_byte_credit =
		uc_port_stats.debug_actual_byte_credit;
	port_stat->debug_actual_packet_credit =
		uc_port_stats.debug_actual_packet_credit;
	port_stat->debug_back_pressure_status =
		uc_port_stats.debug_back_pressure_status;
	port_stat->total_green_bytes = uc_port_stats.total_green_bytes;
	port_stat->total_yellow_bytes = uc_port_stats.total_yellow_bytes;
 ...
 * Return: 0 on success
 */
s32 pp_qos_port_stat_get(struct pp_qos_dev *qdev, u32 id,
			 struct pp_qos_port_stat *port_stat)
{
	s32 rc;
	...
	rc = _qos_port_stat_get(qdev, id, port_stat, port_stat->reset);
out:
	QOS_UNLOCK(qdev);
	return rc;
}
...
/**
 * pp_qos_sf_hist_get() - Get histogram
 * @qos_dev: handle to qos device instance obtained previously from qos_dev_init
 * @sf_id: service flow
 * @hist: pointer to struct to be filled with histogram
 * @reset: reset stats after reading
 *
 * Return: 0 on success
 */
s32 pp_qos_sf_hist_get(struct pp_qos_dev *qdev, u8 sf_id,
	struct pp_sf_hist_stat *hist, bool reset)
{
	struct hist_stats_s uc_hist = { 0 };
	s32 rc = -EINVAL;

	QOS_LOCK(qdev);
	PP_QOS_ENTER_FUNC();
	if (!qos_device_ready(qdev) || ptr_is_null(hist))
		goto out;

	if (unlikely(sf_id >= PP_QOS_MAX_SERVICE_FLOWS)) {
		QOS_LOG_ERR("SF %u is invalid (max %u)\n",
			    sf_id, PP_QOS_MAX_SERVICE_FLOWS);
		goto out;
	}

	create_get_hist_stats_cmd(qdev, sf_id, &uc_hist, reset);
...
EXPORT_SYMBOL(pp_qos_sf_hist_get);
...
static s32 update_num_fast_queues(struct pp_qos_dev *qdev, u8 curr_is_fast,
				  u8 new_is_fast)
{
	if (curr_is_fast == new_is_fast)
		return 0;

	if (new_is_fast) {
		if (qdev->num_fast_queues >= PP_QOS_MAX_FAST_QUEUES) {
			pr_err("Num Fast Queues exceeding maximum\n");
			return -EINVAL;
		}
				   ...
				   u64 *modified)
{
	...
	if (!node->data.queue.is_alias &&
	    node->data.queue.is_fast_q != conf->is_fast_q) {
		/* Fallback to non fast queue if more than 128 defined */
		fast_q_fallback = (conf->is_fast_q &&
			qdev->num_fast_queues >= PP_QOS_MAX_FAST_QUEUES) ?
			true : false;

		if (fast_q_fallback) {
			pr_debug("fallback to non fast q\n");
		} else {
			if (update_num_fast_queues(qdev,
						   node->data.queue.is_fast_q,
						   conf->is_fast_q))
				return -EINVAL;

			node->data.queue.is_fast_q = !!conf->is_fast_q;
			QOS_BITS_SET(*modified, QOS_MODIFIED_FAST_Q);
		}
	}
...
static s32 check_queue_conf_validity(struct pp_qos_dev *qdev, u32 id,
				     const struct pp_qos_queue_conf *conf,
				     struct qos_node *node, u64 *modified,
				     bool syncq, u16 rlm)
{
	u32 phy, mapped_rlm;
	s32 rc;
...
void pp_qos_queue_conf_set_default(struct pp_qos_queue_conf *conf)
{
	...
	conf->ebs = QOS_DEFAULT_MAX_BURST;
	conf->is_fast_q = 0;
}
				     ...
				     u32 *phy_ids, u32 count)
{
	...
	if (ptr_is_null(ids) || ptr_is_null(phy_ids))
		return -EINVAL;
...
s32 qos_sync_queue_disconnect(struct pp_qos_dev *qdev, u32 id)
{
	...
	rc = __qos_queue_stat_get(qdev, id, &stat, false);
	if (unlikely(rc)) {
				  ...
				  struct pp_qos_queue_conf *conf)
{
	...
	conf->is_fast_q = node->data.queue.is_fast_q;
				  ...
				  const struct pp_qos_queue_conf *conf)
{
	...
	pr_buf_cat(buf, QOS_LOG_API_STR_MAX, len,
		   " min_guaranteed=%u max_allowed=%u"
		   " codel_en=%u eir=%u ebs=%u fast=%u%s\n",
		   conf->wred_min_guaranteed,
		   conf->wred_max_allowed,
		   conf->codel_en,
		   conf->eir,
...
static s32 _pp_qos_queue_remove(struct pp_qos_dev *qdev, u32 id)
{
	...
	is_fast_q = node->data.queue.is_fast_q;
	rep_id = get_rep_id_from_id(qdev->mapping, id);
	...
	if (is_fast_q)
		update_num_fast_queues(qdev, is_fast_q, false);
...
no_port_move:
	...
	if (QOS_PHY_VALID(phy) &&
		...
		QOS_LOG_ERR("Prio %u is occupied by phy %u id %d, id from phy : %d\n",
			    conf->queue_child_prop.priority, phy, id,
...
queue_set_cont:
	...
	/* fast queue configuration cannot be changed at this stage
	 * due to HW restrictions, setting it to always false.
	 */
	temp_conf.is_fast_q = 0;
			  ...
			  struct pp_qos_queue_stat *stat)
{
	...
	rc = __qos_queue_stat_get(qdev, id, stat, stat->reset);
out:
	QOS_UNLOCK(qdev);
	return rc;
}
...
s32 pp_qos_codel_cfg_set(struct pp_qos_dev *qdev, struct pp_qos_codel_cfg *cfg)
{
	...
	if (!memcmp(&(qdev->codel_cfg), cfg, sizeof(*cfg)))
		goto out;

	memcpy(&(qdev->codel_cfg), cfg, sizeof(*cfg));
...
s32 pp_qos_codel_cfg_get(struct pp_qos_dev *qdev, struct pp_qos_codel_cfg *cfg)
{
	...
	memcpy(cfg, &(qdev->codel_cfg), sizeof(*cfg));

out:
	QOS_UNLOCK(qdev);
	return rc;
}
EXPORT_SYMBOL(pp_qos_codel_cfg_get);
...
{
	struct qos_node *node;
	struct codel_qstats_s uc_codel_stat = {0};
	s32 rc;
	...
	stat->bytes_dropped = uc_codel_stat.drop_bytes;
	stat->max_packet_size = uc_codel_stat.max_pkt_sz;
	stat->max_sojourn_time = uc_codel_stat.max_packet_sojourn_time;
	stat->min_sojourn_time = uc_codel_stat.min_packet_sojourn_time;
	stat->packets_dropped = uc_codel_stat.drop_pkts;
	stat->total_packets = uc_codel_stat.num_pkts;
	stat->total_sojourn_time = uc_codel_stat.total_sojourn_time;

	return 0;
}

/**
 * pp_qos_codel_queue_stat_get() - Get queue's statistics
 * @qos_dev: handle to qos device instance obtained previously from qos_dev_init
 * @id:	  queue's id obtained from queue_allocate
 * @stat: pointer to struct to be filled with queue's statistics
 *
				...
				struct pp_qos_codel_stat *stat)
{
	...
	rc = __qos_codel_queue_stat_get(qdev, id, stat, stat->reset);
out:
	QOS_UNLOCK(qdev);
	return rc;
}
		    ...
		    bool reset_stats)
{
	...
	for_each_qos_queue(qos_dev, node) {
		...
		rc = __qos_queue_stat_get(qos_dev, id, &qstats, reset_stats);
		if (rc)
			return rc;

		memset(&cstats, 0, sizeof(cstats));
		rc = __qos_codel_queue_stat_get(qos_dev, id, &cstats, reset_stats);
		if (unlikely(rc))
			return rc;
...
s32 pp_qos_aqm_lld_sf_set(struct pp_qos_dev *qdev, u8 sf_id,
			  struct pp_qos_aqm_lld_sf_config *sf_cfg)
{
	s32 rc = 0;
	u32 rlms[PP_QOS_CONTEXT_MAX_QUEUES];
	...
	if (unlikely(sf_cfg->num_queues > PP_QOS_CONTEXT_MAX_QUEUES)) {
		QOS_LOG_ERR("SF %u. Num queues %u invalid (max %u)\n",
			    sf_id, sf_cfg->num_queues,
			    PP_QOS_CONTEXT_MAX_QUEUES);
		rc = -EINVAL;
		goto out;
	}
	...
	create_aqm_sf_set_cmd(qdev, sf_id,
		(sf_cfg->aqm_mode == PP_QOS_AQM_MODE_NORMAL ?
		AQM_MODE_NORMAL : AQM_MODE_NO_DROP), sf_cfg, rlms);
	update_cmd_id(&qdev->drvcmds);
	rc = transmit_cmds(qdev);
			       ...
			       struct pp_qos_aqm_lld_sf_config *sf_cfg)
{
	...
	for (queue_idx = 0; queue_idx < sf_cfg->num_queues; queue_idx++) {
		...
		node->data.queue.aqm_context = PP_QOS_MAX_SERVICE_FLOWS;
...
s32 pp_qos_aqm_lld_sf_remove(struct pp_qos_dev *qdev, u8 sf_id,
			     struct pp_qos_aqm_lld_sf_config *sf_cfg)
{
	s32 rc = 0;
	u32 rlms[PP_QOS_CONTEXT_MAX_QUEUES];
	...
	create_aqm_sf_set_cmd(qdev, sf_id, 0, sf_cfg, rlms);
	update_cmd_id(&qdev->drvcmds);
...
EXPORT_SYMBOL(pp_qos_aqm_lld_sf_remove);
...
/* assuming the conf->sched_child_prop.parent is the actual parent */
static s32 _pp_qos_sched_set(struct pp_qos_dev *qdev, u32 id,
			     const struct pp_qos_sched_conf *conf)
{
	s32 rc;
	...
	if (QOS_PHY_VALID(phy) &&
	    id != get_id_from_phy(qdev->mapping, phy)) {
		QOS_LOG_ERR("Prio %u is occupied by phy %u id =%d\n",
			    conf->sched_child_prop.priority, phy, id);
		rc = -EINVAL;
		return rc;
	}
...
static void _pp_qos_create_s_grp(struct pp_qos_dev *qdev,
				 struct pp_qos_common_node_properties *prop)
{
	u32 id;

	if (_qos_shared_limit_group_add(qdev, QOS_DEFAULT_MAX_BURST, &id)) {
		QOS_LOG_ERR("Failed to create shared bwl group\n");
		return;
	}
	prop->shared_bw_group = id;
}

/**
 * @brief handle sched bwl change, when there is replica to the sched need to
 * make sure that this sched and his replica shared their bw.
 * @param is_rem updating to true in case that need to remove the group
 * @param s_group_id shared group id to remove
 */
static void _pp_qos_handle_sched_bwl_change(struct pp_qos_dev *qdev, u32 p_phy,
					    u32 id,
					    struct pp_qos_sched_conf *conf,
					    bool *is_rem, u32 *s_group_id)
{
	u32 phy, conf_bwl, cur_bwl;
	bool is_new;
	struct qos_node *node = NULL;

	*is_rem = false;
	phy = get_phy_from_id(qdev->mapping, id);
	is_new = !QOS_PHY_VALID(phy);
	conf_bwl = conf->common_prop.bandwidth_limit;

	/* making sure the shared_bw_group is not changed by conf */
	if (is_new) {
		conf->common_prop.shared_bw_group = NO_SHARED_BW_GRP;
	} else {
		node = get_node_from_phy(qdev->nodes, phy);
		if (ptr_is_null(node))
			return;
		conf->common_prop.shared_bw_group = node->shared_bw_group;
	}

	/* if there is no replica - nothing to do */
	if (!is_tree_replica_exists(qdev,
				    get_id_from_phy(qdev->mapping, p_phy)))
		return;

	if (is_new) {
		if (conf_bwl != 0)
			_pp_qos_create_s_grp(qdev, &conf->common_prop);
		return;
	}

	cur_bwl = node->bandwidth_limit;
	/* if the bwl didn't change, nothing to do */
	if (conf_bwl == cur_bwl)
		return;

	if (conf_bwl != 0) {
		/* case of adding bwl from 0, so need to create shared group */
		if (cur_bwl == 0)
			_pp_qos_create_s_grp(qdev, &conf->common_prop);
		return;
	}

	/* case that the bwl changed to 0, so need to remove the shared group */
	conf->common_prop.shared_bw_group = NO_SHARED_BW_GRP;
	*s_group_id = node->shared_bw_group;
	*is_rem = true;
}
		     ...
		     const struct pp_qos_sched_conf *conf)
{
	...
	node = get_node_from_phy(qdev->nodes, phy);
	if (ptr_is_null(node) || node_rep(node) || node_internal(node)) {
		QOS_LOG_ERR("Parent %u is not valid\n",
			    conf->sched_child_prop.parent);
		rc = -EINVAL;
		goto out;
	...
	temp_conf.sched_child_prop.parent =
		_get_actual_parent(qdev, conf->sched_child_prop.parent,
				   conf->sched_child_prop.priority);

	_pp_qos_handle_sched_bwl_change(qdev, phy, id, &temp_conf, &is_rem,
					&shared_group_id);
	...
	temp_conf.sched_child_prop.parent = conf->sched_child_prop.parent;
	rc = _rep_sched_set(qdev, id, &temp_conf);
	if (rc) {
		QOS_LOG_ERR("%s %d\n", __func__, __LINE__);
		goto out;
	}
	...
	if (is_rem)
		_qos_shared_limit_group_remove(qdev, shared_group_id);
	update_cmd_id(&qdev->drvcmds);
	rc = transmit_cmds(qdev);
...
s32 __qos_mem_allocate(struct pp_qos_dev *qdev)
{
	struct hw_mem *mem = &qdev->hwmem;
	u8 *virt;
	dma_addr_t phys;
	u32 sz = 0;

	/* Allocate memory for Queue manager - NIOC */
	mem->qm_ddr_sz =
		PAGE_ALIGN(qdev->init_params.wred_total_avail_resources *
			   PPV4_QOS_DESC_SIZE);
	mem->qm_ddr_virt = pp_dma_alloc(mem->qm_ddr_sz, &mem->qm_ddr_phys,
					PP_DMA_NONECOHERENT);
	if (unlikely(!mem->qm_ddr_virt)) {
		pr_err("Could not allocate %u bytes for qos QM\n",
		       (u32)mem->qm_ddr_sz);
		goto qm_ddr_alloc_err;
	}

	/* Set the required FW-Driver buffers sizes */
	mem->fw_logger_sz = PAGE_ALIGN(PPV4_QOS_LOGGER_BUF_SIZE);
	mem->fw_stats_sz = PAGE_ALIGN(PPV4_QOS_STAT_SIZE);
	mem->max_allow_sz = PAGE_ALIGN(qdev->init_params.max_queues *
				       sizeof(u32));
	mem->wsp_queues_sz = PAGE_ALIGN(sizeof(union wsp_queues_t));
	mem->bwl_sz = PAGE_ALIGN(2 * NUM_OF_NODES * sizeof(u32));
	mem->sbwl_sz = PAGE_ALIGN(NUM_OF_NODES * sizeof(u32));
	mem->dma_buff_sz = PAGE_ALIGN(PPV4_QOS_DMA_BUFF_SIZE);

	/* Allocate memory for FW-Driver - IOC */
	mem->fw_ddr_sz = mem->fw_logger_sz + mem->fw_stats_sz +
			 mem->max_allow_sz + mem->wsp_queues_sz + mem->bwl_sz +
			 mem->sbwl_sz + mem->dma_buff_sz;
	mem->fw_ddr_virt = pp_dma_alloc(mem->fw_ddr_sz, &mem->fw_ddr_phys, 0);
	if (unlikely(!mem->fw_ddr_virt)) {
		pr_err("Failed to allocate %u bytes for qos fw-driver\n",
		       (u32)mem->fw_ddr_sz);
		goto fw_ddr_alloc_err;
	}

	/* Memory allocated, split the pointers */
	virt = (u8 *)mem->fw_ddr_virt;
	phys = mem->fw_ddr_phys;

	/* FW Logger buffer */
	mem->fw_logger_ddr_virt = virt + sz;
	mem->fw_logger_ddr_phys = phys + sz;
	sz += mem->fw_logger_sz;

	/* FW Statistics buffer */
	mem->fw_stats_ddr_virt = virt + sz;
	mem->fw_stats_ddr_phys = phys + sz;
	sz += mem->fw_stats_sz;

	/* Allocate max allowed buffer */
	mem->max_allow_ddr_virt = virt + sz;
	mem->max_allow_ddr_phys = phys + sz;
	sz += mem->max_allow_sz;

	/* WSP queue list buffer + num queues placeholder */
	mem->wsp_queues_ddr_virt = virt + sz;
	mem->wsp_queues_ddr_phys = phys + sz;
	sz += mem->wsp_queues_sz;

	/* BWL allowed buffer.
	 * Buffer contains bw(u32) + max burst(u32) per node
	 */
	mem->bwl_ddr_virt = virt + sz;
	mem->bwl_ddr_phys = phys + sz;
	sz += mem->bwl_sz;

	/* SBWL allowed buffer */
	mem->sbwl_ddr_virt = virt + sz;
	mem->sbwl_ddr_phys = phys + sz;
	sz += mem->sbwl_sz;

	/* Buffer for dma usage */
	mem->dma_buff_virt = virt + sz;
	mem->dma_buff_phys = phys + sz;
	sz += mem->dma_buff_sz;

	qdev->api_str_buf = kzalloc(QOS_LOG_API_STR_MAX, GFP_KERNEL);
	if (unlikely(!qdev->api_str_buf)) {
		pr_err("Could not allocate %u bytes for api string buffer\n",
		       QOS_LOG_API_STR_MAX);
		goto api_str_alloc_err;
	}

	qdev->rlms_container = kcalloc(QOS_MAX_NUM_QUEUES, sizeof(u16),
				       GFP_KERNEL);
	if (unlikely(!qdev->rlms_container)) {
...
rlm_container_alloc_err:
	kfree(qdev->api_str_buf);
api_str_alloc_err:
	/* Free FW memory */
	pp_dma_free(mem->fw_ddr_virt, mem->fw_ddr_sz);
fw_ddr_alloc_err:
	/* Free QM memory */
	pp_dma_free(mem->qm_ddr_virt, mem->qm_ddr_sz);
qm_ddr_alloc_err:
...
{
	/* Free FW memory */
	if (qdev->hwmem.fw_ddr_virt) {
		pp_dma_free(qdev->hwmem.fw_ddr_virt, qdev->hwmem.fw_ddr_sz);
		qdev->hwmem.fw_ddr_virt = NULL;
	}

	/* Free QM memory */
	if (qdev->hwmem.qm_ddr_virt) {
		pp_dma_free(qdev->hwmem.qm_ddr_virt, qdev->hwmem.qm_ddr_sz);
		qdev->hwmem.qm_ddr_virt = NULL;
	}
...
{
	u32 value = PP_REG_RD32((ulong)&qdev->init_params.dccm_db->quanta);

	if (value) {
		*quanta = value;
	} else {
		*quanta = 4;
		QOS_LOG_ERR("quanta wasn't initialize\n");
	}
			       ...
			       unsigned int *num_fast_queues)
{
	...
	if (!qos_device_ready(qdev) || !num_fast_queues) {
		rc = -EINVAL;
		goto out;
	}

	*num_fast_queues = qdev->num_fast_queues;
out:
	QOS_UNLOCK(qdev);
	return rc;
}
EXPORT_SYMBOL(pp_qos_get_num_fast_queues);
		 ...
		 bool create_debugfs)
{
	...
	qos_misc_base_addr = conf->qos_uc_misc;
	...
	qdev->max_bw_limit_kbps =
		QOS_MAX_BW_KBPS_PER_1MHZ_CLK * pp_dev_hw_clk_get();
