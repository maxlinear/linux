...
s32 pp_bmgr_pool_verification(u32 id)
{
	...
	for_each_set_bit(policy_id, pool->policies_bmap, PP_BM_MAX_POLICIES) {
		 ...
		 * other than our pool
		 */
		ret = bmgr_policy_pool_pop_en(policy_id,
					      ~BIT_ULL(pool->pool_id), false);
		if (ret)
			goto free_mem;
		      ...
		      u8 *pool_id, bool lock)
{
	...
	pool->sz = pool_params->num_buffers * pool_params->size_of_buffer;
	...
	if (!pool_params->base_addr_high && !pool_params->base_addr_low) {
		...
		pool->virt = pp_dma_alloc(pool->sz, &pool->phys,
					  PP_DMA_NONECOHERENT);
		if (!pool->virt) {
...
free_memory:
	/* free pointers_table */
	if (pool->internal_ptrs_virt) {
		pp_dma_free(pool->internal_ptrs_virt, pool->internal_ptrs_sz);
		pool->internal_ptrs_sz = 0;
	}
	if (pool->virt) {
		pp_dma_free(pool->virt, pool->sz);
		pool->virt = 0;
	}
	free_pool(*pool_id);
	if (lock)
...
s32 pp_bmgr_pool_base_get(u8 pool_id, void **virt, dma_addr_t *phys, size_t *sz)
{
	s32 ret = 0;
	...
	if (phys)
		*phys = db->pools[pool_id].phys;
	if (virt)
		*virt = db->pools[pool_id].virt;
	if (sz)
		*sz = db->pools[pool_id].sz;
...
void bmgr_pools_per_policy_set(u32 pools_per_policy)
{
	...
	if (pools_per_policy > PP_BM_MAX_POOLS_PER_PLCY) {
		pr_err("Max pools in policy %u exceeds maximum of %d\n",
		       pools_per_policy, PP_BM_MAX_POOLS_PER_PLCY);
		return;
	}
	__bmgr_db_lock();
...
s32 bmgr_config_set(const struct pp_bmgr_init_param * const cfg)
{
	...
	if (cfg->max_pools_in_policy > PP_BM_MAX_POOLS_PER_PLCY) {
		pr_err("Max pools in policy %u exceeds maximum of %d\n",
		       cfg->max_pools_in_policy, PP_BM_MAX_POOLS_PER_PLCY);
		ret = -EINVAL;
		goto reset_cfg_db;
	}
...
s32 pp_bmgr_push_buffer(u32 policy, u32 pool, u64 buff)
{
	...
	if (!bmgr_is_buff_in_pool_range(pool, buff)) {
		pr_err("Buffer pointer %#llx out of pool %u range, abort\n",
		       buff, pool);
		return -EPERM;
	}
