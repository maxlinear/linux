 ...
 * @accl_mode: Defines the default pp acceleration mode.
 */
 ...
 * @tdox_id TCP Ack filtering entry id
...
/**
 * @brief Get destination queue rlm
 * @param id session id
 * @param dest_q pointer to save the destination queue
 * @return s32 0 on success, error code otherwise
 */
...
#else
static inline s32 pp_smgr_init(struct device *dev)
{
	return 0;
}

static inline s32 smgr_sq_init(struct device *dev)
{
	return 0;
}
