...
enum dbg_xsession_mode {
	dbg_xsession_mode_default,
	dbg_xsession_mode_full
};

/**
 * @brief Session manager debugfs dir
 */
struct smgr_dbgfs_db {
	u32 dbg_sess_id;
	enum dbg_xsession_mode dbg_sess_mode;
	u16 mirroring_orig_q;
	u16 mirroring_sess_id;
};
...
{
	struct smgr_dbgfs_db *db;

	if (ptr_is_null(args))
		return;

	db = (struct smgr_dbgfs_db *)args->req_id;
	if (ptr_is_null(db) || !__pp_is_event_valid(args->ev))
		return;

	pr_info("PP event %s(%u): args %p, ret %d\n",
	...
	case PP_SESS_CREATE:
	{
		...
		if (cr_args->base.ret == -EEXIST) {
			...
			db->dbg_sess_id = cr_args->sess_id;
		} else {
...
void __smgr_dbg_inact_sessions_dump(struct seq_file *f)
{
	...
	req.req_prio = 0;
	req.req_id   = (unsigned long)f->private;
...
{
	struct smgr_dbgfs_db *db = data;

	if (ptr_is_null(db))
		return;

	if (kstrtou32(cmd_buf, 10, &db->dbg_sess_id))
		pr_info("failed to parse '%s'\n", cmd_buf);
}
...
void __smgr_dbg_session_si_dump(struct seq_file *f)
{
	...
	struct pp_dsi dsi;
	struct pp_si si;
	struct sess_db_info *info = NULL;
	struct si_ud_frag_remark_info *frag_info;
	char buf[256];
	s32 i;
	...
	if (smgr_session_si_get(db->dbg_sess_id, &si))
		return;
	...
	if (SI_IS_DPU_EXIST(&si)) {
		...
		seq_printf(f, " %-20s: %u\n",
			   ...
			   si.dpu.nhdr_l3_off);
		seq_printf(f, " %-20s: %u\n",
			   pp_si_fld2str(SI_FLD_DPU_NHDR_L4_OFF),
			   si.dpu.nhdr_l4_off);
		seq_printf(f, " %-20s: %u\n",
	...
	if (SI_IS_SCE_EXIST(&si)) {
		...
		seq_printf(f, " %-20s: %#x\n",
			   ...
			   si.sce.l4_csum_zero);
		seq_printf(f, " %-20s: %#x\n",
			   pp_si_fld2str(SI_FLD_SCE_NEW_L3_HDR_SZ),
			   si.sce.new_l3_hdr_sz);
		seq_printf(f, " %-20s: %#x\n",
			   pp_si_fld2str(SI_FLD_SCE_NEW_L4_CSUM),
			   si.sce.new_l4_csum);
		seq_printf(f, " %-20s: %#x\n",
		...
		seq_printf(f, " %-20s: %u(%#x)\n",
			   pp_si_fld2str(SI_FLD_SCE_L2TP_DIFF),
			   si.sce.l2tp_diff, si.sce.l2tp_diff);
		seq_printf(f, " %-20s: %u(%#x)\n",
	...
	if (SI_IS_BCE_EXIST(&si)) {
		...
		seq_printf(f, " %-20s: ", "New Header");
		seq_hex_dump(f, "", DUMP_PREFIX_NONE, 32, 1, si.bce.nhdr,
			     min_t(u32, si.dpu.nhdr_sz, 32), false);
		if (si.dpu.nhdr_sz > 32)
			seq_hex_dump(f, "                       ",
				     DUMP_PREFIX_NONE, 32, 1, si.bce.nhdr + 32,
				     si.dpu.nhdr_sz - 32, false);
	}
	if (si.si_ud_sz || si.si_ps_sz) {
		...
		if (smgr_session_info_get(db->dbg_sess_id, info))
			goto done;

		if (test_bit(SESS_FLAG_MTU_CHCK, &info->flags)) {
			frag_info = (struct si_ud_frag_remark_info *)
				    &si.ud[PP_PS_REGION_SZ];
			seq_printf(f, " %-20s: %u (INT_DF %d (IGNORE %d), EXT_DF %d (IGNORE %d), FRAG_EXT %d, IPV4 %d, PPPOE %d, REMARK %d)\n",
				   "frag.flags", frag_info->frag.flags,
				   !!(frag_info->frag.flags &
				   ...
				   !!(frag_info->frag.flags &
				      FRAG_INFO_FLAG_REMARK));
			seq_printf(f, " %-20s: %u\n", "frag.l3_off",
				   frag_info->frag.l3_off);
			seq_printf(f, " %-20s: %u\n", "frag.dst_q",
				   frag_info->dst_q);
			seq_printf(f, " %-20s: %u\n", "frag.max_pkt_size",
				   frag_info->frag.max_pkt_size);
		}
	}
	...
	if (smgr_session_dsi_get(db->dbg_sess_id, &dsi))
		goto done;
...
void __smgr_dbg_exception_session_si_dump(struct seq_file *f)
{
	...
	ret = chk_exception_session_si_get(db->dbg_sess_id, &si);
	if (unlikely(ret)) {
...
void __smgr_dbg_session_si_raw_dump(struct seq_file *f)
{
	...
	ret = cls_session_si_get(db->dbg_sess_id, &hw_si);
	if (ret)
		return;
	ret = chk_session_dsi_get(db->dbg_sess_id, &hw_dsi);
	if (ret)
		return;
			       ...
			       struct pp_si *si)
{
	...
	if (SI_IS_DPU_EXIST(si)) {
		...
		seq_printf(f, " %-20s = %u[%#x]\n", "New Header L4 Offset",
			   si->dpu.nhdr_l4_off, si->dpu.nhdr_l4_off);
		seq_printf(f, " %-20s = %u[%#x]\n", "Layer Field Offset",
 ...
 *        counters
 */
void __smgr_dbg_xsession_dump(struct seq_file *f)
{
	...
	if (info->tdox_id != U16_MAX) {
		seq_puts(f,   " TDOX\n");
		seq_puts(f,   "========\n");
		seq_printf(f, " %-20s = %u\n","ID", info->tdox_id);
		if (si.tdox_flow < UC_MAX_TDOX_SESSIONS)
			seq_printf(f, " %-20s = %u\n","SUPP", si.tdox_flow);
	}
...
PP_DEFINE_DEBUGFS(xsession, __smgr_dbg_xsession_dump, __smgr_dbg_session_set);

#define PP_DBG_XSESSION_ALL_LIMIT	10

/**
 * @brief set function to configure xsession_all print mode
 */
void __smgr_dbg_xsession_all_set(char *cmd_buf, void *data)
{
	struct smgr_dbgfs_db *db = data;
	u32 mode = dbg_xsession_mode_default;

	if (kstrtou32(cmd_buf, 10, &mode))
		pr_info("failed to parse '%s'\n", cmd_buf);

	if (mode != dbg_xsession_mode_default && mode != dbg_xsession_mode_full) {
		pr_info("\n");
		pr_info(" Usage: echo <option> > xsession_all\n");
		pr_info(" Options:\n");
		pr_info("   %d : limited (default) - output print is limited to first %d sessions\n",
			dbg_xsession_mode_default, PP_DBG_XSESSION_ALL_LIMIT);
		pr_info("   %d : full bitmap mode  - print all open sessions\n", dbg_xsession_mode_full);
		return;
	}

	db->dbg_sess_mode = (enum dbg_xsession_mode)mode;
}

/**
 * @brief Dump session info for all open sessions
 */
void __smgr_dbg_xsession_all_dump(struct seq_file *f)
{
	struct smgr_dbgfs_db *db = f->private;
	unsigned long *bmap;
	u32 n_sessions;
...
PP_DEFINE_DEBUGFS(xsession_all, __smgr_dbg_xsession_all_dump,
		__smgr_dbg_xsession_all_set);
...
void __smgr_dbg_stats_dump(struct seq_file *f)
{
	...
	seq_printf(f, " | %-28s | %-10u | %-28s | %-10u |\n",
		   ...
		   "HAL Error",
		   atomic_read(&stats.hal_err));
		   seq_printf(f, " | %-28s | %-10u | %-28s | %-10s |\n",
		   "HW Table Full",
		   atomic_read(&stats.hw_table_full),
		   "", "");
	seq_puts(f,   " +------------------------------+------------+------------------------------+------------+\n");
	seq_puts(f,   "\n");
}
 ...
 * @return 0 on success, non-zero value otherwise
 */
static int __smgr_dbg_open_frag_sess_set(void *data, u64 val)
{
	return smgr_open_frag_sess_set(val ? true : false);
}
 ...
 * @return 0 on success, non-zero value otherwise
 */
static int __smgr_dbg_open_frag_sess_get(void *data, u64 *val)
{
	*val = (u64)smgr_open_frag_sess_get();
	return 0;
}

PP_DEFINE_DBGFS_ATT(open_frag_sess, __smgr_dbg_open_frag_sess_get,
				    __smgr_dbg_open_frag_sess_set);

/**
 * @brief Enable/Disable open lld session flag
 * @param data not used
 * @param val 1 to enable, 0 to disable
 * @return 0 on success, non-zero value otherwise
 */
static int __smgr_dbg_open_lld_sess_set(void *data, u64 val)
{
	return smgr_open_lld_sess_set(val ? true : false);
}

/**
 * @brief Get session manager open lld enable/disable state
 * @param data unused
 * @param val enable/disable
 * @return 0 on success, non-zero value otherwise
 */
static int __smgr_dbg_open_lld_sess_get(void *data, u64 *val)
{
	*val = (u64)smgr_open_lld_sess_get();
	return 0;
}

PP_DEFINE_DBGFS_ATT(open_lld_sess, __smgr_dbg_open_lld_sess_get,
				    __smgr_dbg_open_lld_sess_set);

enum sess_set_dq_opts {
	sess_set_dq_opt_help = 1,
	sess_set_dq_opt_id,
...
void __smgr_dbg_session_compare_last_fv_dump(struct seq_file *f)
{
	...
	if (smgr_session_info_get(db->dbg_sess_id, &info)) {
		pr_err("Session %u does not exist!", db->dbg_sess_id);
		return;
	}
	...
	for (i = 0; i < PP_MAX_FV_WORD_SIZE; i++) {
		match = true;
		...
		if (i < offset) {
			...
			seq_printf(f, " |   FV WORD%02u   | 0x00000000 | %#010x |  %c  |\n",
				   i, fv[i], match ? ' ' : 'X');
		} else {
	...
	if (info.hash.h1 != last_hash.h1) {
		full_match = false;
		match = false;
	}
	seq_printf(f, " |   HASH1       | %#010x | %#010x |  %c  |\n", info.hash.h1,
		   last_hash.h1, match ? ' ' : 'X');
	if (info.hash.h2 != last_hash.h2) {
		full_match = false;
		match = false;
	}
	seq_printf(f, " |   HASH2       | %#010x | %#010x |  %c  |\n", info.hash.h2,
		   last_hash.h2, match ? ' ' : 'X');
	if (info.hash.sig != last_hash.sig) {
		full_match = false;
		match = false;
	}
	seq_printf(f, " |   Signature   | %#010x | %#010x |  %c  |\n",
		   info.hash.sig, last_hash.sig, match ? ' ' : 'X');
	seq_puts(f, " +---------------+------------+------------+-----+\n\n");
...
void sess_mirroring_set(char *cmd_buf, void *data)
{
	...
	if (cmd.enable) {
		if (db->mirroring_orig_q != U16_MAX) {
			pr_info("Error: Session %u mirroring alrady in use\n",
				db->mirroring_sess_id);
			return;
		}
		...
		db->mirroring_orig_q = cmd.orig_q;
		db->mirroring_sess_id = sess_id;
		...
		if (ret) {
			...
			db->mirroring_orig_q = U16_MAX;
			cmd.enable = 0;
			uc_egr_mbox_cmd_send(UC_CMD_MCAST_SESS_MIRRORING, 0,
					     (const void *)&cmd, sizeof(cmd),
					     NULL, 0);
			db->mirroring_sess_id = U16_MAX;
			return;
		}
		...
		ret = chk_session_dsi_q_update(db->mirroring_sess_id,
					       db->mirroring_orig_q);
		if (unlikely(ret)) {
			pr_err("failed to update session %u dest queue to %u\n",
			       db->mirroring_sess_id, db->mirroring_orig_q);
			return;
		}
...
{
	seq_puts(f, "Enable:\n");
	seq_puts(f, "echo en=1 id=[sess_id] q=[phy queue id] gpid=[new_gpid] > sess_mirroring\n");
	seq_puts(f, "Disable:\n");
...
static const char *accl_mode[PP_ACCL_MODE_NUM] = {
	"Normal Mode",
	"Unknown L4 Mode",
	"Unknown L3 Mode"
};

static void sess_accel_mode_set_help(void)
{
	pr_info("\n");
	pr_info(" Brief: Set PP acceleration mode\n");
	pr_info(" Usage: echo <mode>[=value] > acceleration_mode\n");
	pr_info(" Options:\n");
	pr_info("   0  - Normal Mode\n");
	pr_info("   1  - Unknown L4 Mode\n");
	pr_info("   2  - Unknown L3 Mode\n");
	pr_info(" Examples:\n");
	pr_info("   set acceleration mode to 'Unknown L4':\n");
	pr_info("     echo 1 > acceleration_mode\n");
}

static void sess_accel_mode_set(char *cmd_buf, void *data)
{
	u32 mode;

	if (sscanf(cmd_buf, "%u", &mode) != 1) {
		sess_accel_mode_set_help();
		return;
	}

	if (pp_accl_mode_set((u8)mode))
		pr_err("failed to set pp acceleration mode: %u\n", mode);
	else
		pr_info("Acceleration mode: %s set\n", accl_mode[mode]);
}

static void sess_accel_mode_get(struct seq_file *f)
{
	u8 mode;

	if (pp_accl_mode_get(&mode)) {
		pr_err("failed to get pp acceleration mode\n");
		return;
	}

	seq_puts(f, "\n");
	seq_printf(f, "PP Acceleration Mode: %s\n", accl_mode[mode]);
}

PP_DEFINE_DEBUGFS(acceleration_mode, sess_accel_mode_get, sess_accel_mode_set);

static struct debugfs_file debugfs_files[] = {
	{ "sessions", &PP_DEBUGFS_FOPS(sessions) },
	{ "inact_sessions", &PP_DEBUGFS_FOPS(inact_sessions) },
	{ "xsession", &PP_DEBUGFS_FOPS(xsession) },
	{ "xsession_all", &PP_DEBUGFS_FOPS(xsession_all) },
	...
	{ "acceleration_mode", &PP_DEBUGFS_FOPS(acceleration_mode) },
};
...
s32 smgr_dbg_init(struct device *dev, struct dentry *parent)
{
	...
	ret = smgr_tdox_create_debugfs(dbgfs);
	if (unlikely(ret))
		goto done;

	ret = smgr_lro_create_debugfs(dbgfs);
	if (unlikely(ret))
		goto done;
