...
static s32 __proto_mac_init(struct protocol_info *info)
{
	...
	ext->fld[0].len    = sizeof_field(struct ethhdr, h_proto);
...
static s32 __proto_mac_int_init(struct protocol_info *info)
{
	...
	np->len   = sizeof_field(struct ethhdr, h_proto) * BITS_PER_BYTE;
	...
	/* ethertype */
	fld = &params->ext.fld[params->ext.n_fld++];
	fld->len    = sizeof_field(struct ethhdr, h_proto);
...
static void __proto_vlan_common_init(struct protocol_info *info, char *name)
{
	...
	params->ext.fld[0].len  = sizeof_field(struct vlan_hdr, h_vlan_TCI);
	...
	params->ext.fld[0].off  = offsetof(struct vlan_hdr, h_vlan_TCI) +
				  sizeof_field(struct ethhdr, h_proto);
...
static s32 __proto_pppoe_init(struct protocol_info *info)
{
	...
	/* extracting the pppoe code and session id */
	fld = &params->ext.fld[params->ext.n_fld++];
	fld->len    = 2;
	fld->off    = sizeof_field(struct ethhdr, h_proto);
	fld->fv_idx = offsetof(struct pp_fv, tunn.pppoe.code);
	fld->mask   = 0;
	/* PPPoE session id */
	fld = &params->ext.fld[params->ext.n_fld++];
	fld->len    = sizeof_field(struct pppoe_hdr, sid);
	fld->off    = offsetof(struct pppoe_hdr, sid) +
		      sizeof_field(struct ethhdr, h_proto);
	fld->fv_idx = offsetof(struct pp_fv, tunn.pppoe.sess_id);
	fld->mask   = 0;
	__proto_pr_debug(info);
	pr_debug("done\n");
...
/**
 * @brief Init mpls select info, this is not a real protocol and only
 *        used to detect whether the next protocol header is IPv4/IPv6
 *        or inner L2 header
 */
static s32 __proto_mpls_select_init(struct protocol_info *info)
{
	struct prsr_up_layer_proto_params *params;
	struct skip_info *skip;
	struct np_info *np;
	...
	params      = &info->up;
	info->id       = PRSR_PROTO_MPLS_SEL;
	info->prof_id  = PRSR_PROFILE_COMMON;
	info->name     = params->name; /* a reference to the name */
	n = strscpy(params->name, "MPLS Select", sizeof(params->name));
	/* parsing info */
	params->proto_id   = U16_MAX;
	params->hw_idx     = TABLE_ENTRY_INVALID_IDX;
	params->hdr_len    = 1;
	...
	params->entry_type = ETYPE_NONE;
	/* next protocol info
	 * same logic as ip select including the value '0'
	...
	/* preceding protocols */
	params->n_pre = 1;
	/* IP select -> MPLS select (last label) */
	params->pre[0].np_logic = NP_LOGIC_IP_SELECT;
	params->pre[0].val      = 0;
	__proto_pr_debug(info);
	pr_debug("done\n");
...
/**
 * @brief Init mpls info, Since we support up to 4 MPLS labels
 *        we need to define the logic for the parser to detect
 *        the next label.
 */
static s32 __proto_mpls_init(struct protocol_info *info)
{
	struct prsr_up_layer_proto_params *params;
	struct skip_info *skip;
	struct np_info *np;
	...
	params      = &info->up;
	info->id       = PRSR_PROTO_MPLS;
	info->prof_id  = PRSR_PROFILE_COMMON;
	info->name     = params->name; /* a reference to the name */
	n = strscpy(params->name, "MPLS", sizeof(params->name));
	/* set protocol as last ethertype */
	params->n_leth = 1;
	params->leth[0].ethtype = ETH_P_MPLS_UC;
	n = strscpy(params->leth[0].name, params->name,
		    sizeof(params->leth[0].name));
	/* parsing info */
	params->proto_id   = ETH_P_MPLS_UC;
	params->hw_idx     = TABLE_ENTRY_INVALID_IDX;
	params->hdr_len    = 4;
	params->ignore     = true;
	params->entry_type = ETYPE_NONE;
...
static void __proto_ipv4_common_init(struct protocol_info *info, char *name)
{
	...
	np->len   = sizeof_field(struct iphdr, protocol) * BITS_PER_BYTE;
	np->dflt  = PRSR_PROTO_PAYLOAD;
	/* header skip info */
	skip = &params->skip;
...
static void __proto_ipv6_common_init(struct protocol_info *info, char *name)
{
	...
	np->len   = sizeof_field(struct ipv6hdr, nexthdr) * BITS_PER_BYTE;
	np->dflt  = PRSR_PROTO_PAYLOAD;
					   ...
					   char *name)
{
	...
	np->len   = sizeof_field(struct ipv6_opt_hdr, nexthdr) * BITS_PER_BYTE;
	np->dflt  = PRSR_PROTO_PAYLOAD;
	/* header skip info */
	skip = &params->skip;
	skip->hdr_len_off = offsetof(struct ipv6_opt_hdr, hdrlen) *
			    BITS_PER_BYTE;
	skip->hdr_len_sz  = sizeof_field(struct ipv6_opt_hdr, hdrlen) *
			    BITS_PER_BYTE;
	skip->hdr_len_im  = 8;
	skip->logic       = SKIP_LOGIC_ALU;
					  ...
					  char *name)
{
	...
	np->len   = sizeof_field(struct frag_hdr, nexthdr) * BITS_PER_BYTE;
	np->dflt  = PRSR_PROTO_PAYLOAD;
	/* header skip info - IPv6 has constant header size */
	skip = &params->skip;
...
#ifdef CONFIG_PPV4_ICMP_SUPPORT
/**
...
static void __proto_esp_common_init(struct protocol_info *info, char *name)
{
	...
	fld->len  = sizeof_field(struct ip_esp_hdr, spi);
	fld->mask = 0;
}
...
static void __proto_tcp_common_init(struct protocol_info *info, char *name)
{
	...
	skip->hdr_len_off = (offsetof(struct tcphdr, ack_seq) +
			     sizeof_field(struct tcphdr, ack_seq)) *
			    BITS_PER_BYTE;
	/* cannot use sizeof_field(struct tcphdr, doff) cause its bitfield */
	skip->hdr_len_sz = 4;
	skip->logic      = SKIP_LOGIC_ALU;
	skip->op         = ALU_OP_4XLEN_PLUS_IM;
	skip->hdr_len_im = 0;
...
static void __proto_udp_common_init(struct protocol_info *info, char *name)
{
	...
	skip->hdr_len_sz  = sizeof_field(struct udphdr, dest) * BITS_PER_BYTE;
...
static s32 __proto_gre_init(struct protocol_info *info)
{
	...
	np->len   = sizeof_field(struct gre_base_hdr, protocol) * BITS_PER_BYTE;
	np->logic = NP_LOGIC_TUNN;
	np->dflt  = PRSR_PROTO_PAYLOAD;
	/* header skip info */
	skip = &params->skip;
...
static s32 __proto_l2tpv3_ip_init(struct protocol_info *info)
{
	...
	fld->len = L2TPV3_SESS_LEN;
	fld->off = L2TPV3_OIP_SESS_OFF;
	fld->mask = 0;
	__proto_pr_debug(info);
	pr_debug("done\n");
...
static s32 __proto_l2tpv3_udp_init(struct protocol_info *info)
{
	...
	/* header skip info */
	params->skip.logic = SKIP_LOGIC_IM;
	params->skip.hdr_len_im = L2TPV3_OUDP_HDR_SIZE;
	/* preceding protocols */
	params->n_pre = 1;
	/* UDP -> L2TP select */
	params->pre[0].np_logic = NP_LOGIC_UDP;
 ...
 */
static s32 __proto_l2tpv2_select_init(struct protocol_info *info)
{
	struct prsr_up_layer_proto_params *params;
	struct np_info *np;
	s32 n;
	...
	params = &info->up;
	info->id = PRSR_PROTO_L2TPV2_SEL;
	info->prof_id = PRSR_PROFILE_COMMON;
	info->name = params->name;
	n = strscpy(params->name, "L2TPv2 Select", sizeof(params->name));
	/* parsing info */
	params->proto_id = U16_MAX;
	params->hw_idx = TABLE_ENTRY_INVALID_IDX;
	params->hdr_len = 2;
	params->entry_type = ETYPE_NONE;
	params->ignore = true;
	/* we support only length bit in the l2tpv2 header to be on
	 * so verify this, we use the entire 1st byte of the header
	 * |T|L|x|x|S|x|O|P| -> onl support 0x00 and 0x40 values
	 */
	np = &params->np;
	np->off = 0;
	np->len = 8;
	...
	/* preceding protocols */
	params->n_pre = 1;
	/* L2TP select -> L2TPv2 select */
	params->pre[0].np_logic = NP_LOGIC_UDP;
	params->pre[0].val = L2TPV2_VER;
	__proto_pr_debug(info);
	pr_debug("done\n");
...
static s32 __proto_l2tpv2_op_len_init(struct protocol_info *info)
{
	...
	params->proto_id = U16_MAX;
	params->hw_idx = TABLE_ENTRY_INVALID_IDX;
	params->hdr_len = L2TPV2_BASE_HDR_SIZE + L2TPV2_OPT_LEN_LEN;
	params->entry_type = ETYPE_NONE;
	params->ignore = false;
	/* next protocol info */
	np = &params->np;
	np->dflt = PRSR_PROTO_PPP;
	np->logic = NP_LOGIC_END;
	/* header skip info, skip the l2tpv2 header with the length field */
	params->skip.logic = SKIP_LOGIC_IM;
	params->skip.hdr_len_im = L2TPV2_BASE_HDR_SIZE + L2TPV2_OPT_LEN_LEN;
	params->skip.hdr_len_min = L2TPV2_BASE_HDR_SIZE + L2TPV2_OPT_LEN_LEN;
	params->skip.hdr_len_max = 0xFF;
	/* preceding protocols */
	params->n_pre = 1;
...
static s32 __proto_l2tpv2_no_op_len_init(struct protocol_info *info)
{
	...
	params->proto_id = U16_MAX;
	params->hw_idx = TABLE_ENTRY_INVALID_IDX;
	params->hdr_len = L2TPV2_BASE_HDR_SIZE;
	params->entry_type = ETYPE_NONE;
	params->ignore = false;
	/* next protocol info */
	np = &params->np;
	np->dflt = PRSR_PROTO_PPP;
	np->logic = NP_LOGIC_END;
	/* header skip info, skip the l2tpv2 header */
	params->skip.logic = SKIP_LOGIC_IM;
	params->skip.hdr_len_im = L2TPV2_BASE_HDR_SIZE;
	params->skip.hdr_len_min = L2TPV2_BASE_HDR_SIZE;
	params->skip.hdr_len_max = 0xFF;
	/* preceding protocols */
	params->n_pre = 1;
...
static s32 __proto_ppp_init(struct protocol_info *info)
{
	...
	np->dflt = PRSR_PROTO_PAYLOAD;
	/* header skip info */
	params->skip.hdr_len_im = PPP_HDRLEN;
	params->skip.hdr_len_min = PPP_HDRLEN;
	params->skip.hdr_len_max = 0xFF;
	params->skip.logic = SKIP_LOGIC_IM;
	/* preceding protocols */
	__proto_pr_debug(info);
...
static s32 __proto_vxlan_init(struct protocol_info *info)
{
	...
	fld->len    = sizeof_field(struct vxlanhdr, vx_vni);
	fld->mask   = 0;
	__proto_pr_debug(info);
	pr_debug("done\n");
...
static s32 __proto_geneve_init(struct protocol_info *info)
{
	...
	np->len   = sizeof_field(struct genevehdr, proto_type);
	np->dflt  = PRSR_PROTO_MAC_INT;
	/* header skip info */
	skip = &params->skip;
	skip->logic       = SKIP_LOGIC_ALU;
...
#ifdef CONFIG_PPV4_SCTP_SUPPORT
/**
...
s32 __prsr_protocols_init(struct protocols *protos)
{
	...
	INIT_PROTO(__proto_l2tpv2_select_init,    protos, ret, i);
