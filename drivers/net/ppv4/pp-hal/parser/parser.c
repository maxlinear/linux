...
s32 prsr_ipv6_nexthdr_support_set(u32 nexthdr, bool en)
{
	...
	reg = PP_FIELD_MOD(BIT(nexthdr % BITS_PER_U32), !en, reg);
...
s32 prsr_ipv6_nexthdr_support_get(u32 nexthdr, bool *en)
{
	...
	reg = PP_REG_RD32(PRSR_IPV6_NEXTHDR_TO_REG(nexthdr));
	*en = !PP_FIELD_GET(BIT(nexthdr % BITS_PER_U32), reg);
...
s32 prsr_ipv6_nexthdr_support_get_all(unsigned long *nexthdr_map, u32 map_sz)
{
	...
	if (unlikely(ptr_is_null(nexthdr_map)))
		return -EINVAL;
	if (unlikely(map_sz != NEXTHDR_MAX+1))
...
static bool prsr_port_entry_is_valid(struct port_cfg_entry *ent)
{
	...
	tot_len  = 0;
	max_len  = sizeof_field(struct pp_fv_oob, stw_data) * BITS_PER_BYTE - 1;
...
static void __prsr_stats_get(struct prsr_stats *stats)
{
	...
	stats->rpb_discard    =
		PP_REG_RD32(PP_PRSR_RPB_DISCARD_CNT_REG);
	stats->err.rpb_err        =
		PP_REG_RD32(PP_PRSR_RPB_ERROR_CNT_REG);
	stats->zero_len       =
		PP_REG_RD32(PP_PRSR_ZERO_LEN_CNT_REG);
	stats->err.proto_len_err  =
		PP_REG_RD32(PP_PRSR_PROTO_LENGTH_ERR_CNT_REG);
	stats->err.hdr_len_err    =
		PP_REG_RD32(PP_PRSR_MAX_HEADER_LEN_ERR_CNT_REG);
	stats->err.proto_overflow =
		PP_REG_RD32(PP_PRSR_PROTO_OVERFLOW_ERR_CNT_REG);
	stats->err.ip_len_err     =
		PP_REG_RD32(PP_PRSR_IP_LEN_ERR_CNT_REG);
	stats->err.payload_err    =
		PP_REG_RD32(PP_PRSR_EOF_NO_PAYLOAD_ERR_CNT_REG);
	stats->aborted_pkts   =
		PP_REG_RD32(PP_PRSR_ABORT_CNT_ERR_CNT_REG);
	stats->err.last_eth_err =
