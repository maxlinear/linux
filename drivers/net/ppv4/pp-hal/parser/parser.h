...
struct prsr_stats {
	...
	struct {
		 ...
		 */
		u32 proto_len_err;
		/*! number of packets that layer 3 header was bigger or
		 *  smaller than the values configured in the imem table
		 */
		u32 hdr_len_err;
		/*! number of packets with protocol overflow error */
		u32 proto_overflow;
		/*! number of packets with a mismatch between the IP
		 *  header length and packet length reported by the RPB
		 */
		u32 ip_len_err;
