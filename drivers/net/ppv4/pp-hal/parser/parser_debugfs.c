...
void __prsr_stats_show(struct seq_file *f)
{
	...
	seq_printf(f, " | %-25s | %10u |\n", "L3 Hdr Exceeds Min-Max",
		   st.err.proto_len_err);
	seq_puts(f,   " +---------------------------+------------+\n");
	seq_printf(f, " | %-25s | %10u |\n", "L3 Hdr Longer Than Cfg",
		   st.err.hdr_len_err);
	seq_puts(f,   " +---------------------------+------------+\n");
	seq_printf(f, " | %-25s | %10u |\n", "Protocol Overflow",
		   st.err.proto_overflow);
	seq_puts(f,   " +---------------------------+------------+\n");
	seq_printf(f, " | %-25s | %10u |\n", "IP Length Error",
		   st.err.ip_len_err);
	seq_puts(f,   " +---------------------------+------------+\n");
...
void __prsr_global_cfg_show(struct seq_file *f)
{
	...
	unsigned long err_flags, ipv6_supp[BITS_TO_LONGS(NEXTHDR_MAX+1)];
	...
	if (unlikely(ret))
		return;
	ret = prsr_ipv6_nexthdr_support_get_all(ipv6_supp, NEXTHDR_MAX+1);
	if (unlikely(ret))
		return;
	pr_buf(buf, sizeof(buf), n, "%*pbl", NEXTHDR_MAX+1, ipv6_supp);
			     ...
			     u32 n, bool header, bool footer)
{
	...
	for (i = start; i < start + n; i++) {
		...
		seq_printf(f, " | %3u", test_bit(i, hit_map));
		seq_printf(f, " | %-7u | %-3u | %-4u | %-3u | %u-%-11s",
			      ...
			      enum pp_fv_proto_info pinfo, u8 l3_proto)
{
	if (PP_FV_IS_L3_IPV4(pinfo)) {
		switch (l3_proto) {
		case IPPROTO_ESP:
			seq_printf(f, "| Layer 4 | ESP SPI          | %#-39x |\n",
				   ntohl(l4->esp.spi));
			break;
		case IPPROTO_ICMP:
		case IPPROTO_ICMPV6:
 ...
 */
static void __prsr_fv_tunn_show(struct seq_file *f, struct pp_fv *fv,
				enum pp_fv_proto_info pinfo, u8 l3_proto,
				u16 udp_dst_port)
{
	if (PP_FV_IS_L4_UDP(pinfo)) {
		...
		case VXLAN_UDP_DPORT:
			seq_printf(f, "| Tunnel  | VNI              | %-39x |\n",
				   ntohl(fv->tunn.vxlan.vni));
			return;
		case GENEVE_UDP_DPORT:
			seq_printf(f, "| Tunnel  | VNI              | %02x,%02x,%02x%32s |\n",
				   fv->tunn.geneve.vni[2],
				   fv->tunn.geneve.vni[1],
				   fv->tunn.geneve.vni[0], "");
			return;
		case L2TP_UDP_DPORT:
			seq_printf(f, "| Tunnel  | Tunnel ID        | %-39x |\n",
				   ntohs(fv->tunn.l2tpoudp.tunn_id));
			seq_printf(f, "|         | Session ID       | %-39x |\n",
				   ntohs(fv->tunn.l2tpoudp.sess_id));
			return;
		default:
			break;
		}
	}
	if (ntohs(fv->first.l2.h_prot) == PPP_IP   ||
	    ntohs(fv->first.l2.h_prot) == PPP_IPV6 ||
	    ntohs(fv->second.l2.h_prot) == PPP_IP  ||
	    ntohs(fv->second.l2.h_prot) == PPP_IPV6) {
		seq_printf(f, "| Tunnel  | PPPoE code       | %-39x |\n",
				   ntohs(fv->tunn.pppoe.code));
		seq_printf(f, "|         | PPPoE Session ID | %-39x |\n",
				   ntohs(fv->tunn.pppoe.sess_id));
	}
}
...
/**
 * @brief Update is_err value if it's error packet according to stw and return 0
 *  In case of failure to read from STW, returns -EINVAL
 */
static s32 __prsr_last_fv_is_err(bool *is_err)
{
	struct prsr_hw_stw hw_stw;
	struct buf_fld_info err_fld = prsr_stw_flds[STW_PRSR_FLD_ERROR_IND];

	if (unlikely(prsr_last_stw_get(&hw_stw)))
			return -EINVAL;

	*is_err = buf_fld_get(&err_fld, hw_stw.word);
	return 0;
}
...
void __prsr_fv_show(struct seq_file *f)
{
	...
	bool is_err_pkt = false;

	if (__prsr_last_fv_is_err(&is_err_pkt)){
		seq_puts(f, "failed to get the STW error indication\n");
		return;
	}
	...
	seq_puts(f,   "+----------------------------------------------------------------------+\n");
	if (is_err_pkt)
		seq_puts(f,   "|                Field Vector Masked  (Error Packet)                   |\n");
	else
	...
	seq_puts(f,   "+---------+------------------+-----------------------------------------+\n");
	__prsr_fv_tunn_show(f, &fv, ext_pinfo, fv.first.l3.v4.protocol,
			    fv.first.l4.udp.dst);
	seq_puts(f,   "+---------+------------------+-----------------------------------------+\n");
