...
static void __fv_l4_set(struct pktprs_hdr *pkt, struct pp_fv *fv, u8 lvl)
{
	...
	if (p.icmp6) {
		l4->icmp.code = p.icmp6->icmp6_code;
 ...
 */
static void __fv_tunn_lvl_set(struct pktprs_hdr *pkt, struct pp_fv *fv)
{
	union pp_fv_tunnel *tun = &fv->tunn;
	union proto_ptr p;

	p.vxlan = pktprs_vxlan_hdr(pkt, PKTPRS_HDR_LEVEL0);
	if (p.vxlan) {
		tun->vxlan.vni = p.vxlan->vx_vni;
		return;
	}

	p.geneve = pktprs_geneve_hdr(pkt, PKTPRS_HDR_LEVEL0);
	if (p.geneve) {
	...
	p.l2tp = pktprs_l2tp_oudp_hdr(pkt, PKTPRS_HDR_LEVEL0);
	if (p.l2tp) {
		if ((ntohs(p.l2tp->v2.flags) & L2TP_HDR_VER_MASK) ==
		    L2TP_HDR_VER_2) {
			if (ntohs(p.l2tp->v2.flags) & L2TP_HDRFLAG_L) {
	...
	/* if inner pppoe, use the inner for FV */
	p.pppoe = pktprs_pppoe_hdr(pkt, PKTPRS_HDR_LEVEL1);
	if (p.pppoe) {
		tun->pppoe.code = *((__be16 *)p.pppoe);
		tun->pppoe.sess_id = p.pppoe->sid;
		return;
	}
	/* if only outer pppoe and no other tunnel, use the pppoe for FV */
	p.pppoe = pktprs_pppoe_hdr(pkt, PKTPRS_HDR_LEVEL0);
	if (p.pppoe) {
		tun->pppoe.code = *((__be16 *)p.pppoe);
		tun->pppoe.sess_id = p.pppoe->sid;
		return;
...
u8 pp_fv_proto_set(struct pktprs_hdr *pkt, u8 lvl)
{
	...
	other_l4 = PKTPRS_IS_ESP(pkt, lvl);
	if (!lvl) {
		other_l4 =
#ifdef CONFIG_PPV4_SCTP_SUPPORT
			   PKTPRS_IS_SCTP(pkt, lvl) ||
#endif
#ifdef CONFIG_PPV4_ICMP_SUPPORT
