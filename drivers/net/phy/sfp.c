...
static int sfp_debug_update_state_show(struct seq_file *s, void *data)
{
	struct sfp *sfp = s->private;
...
DEFINE_SHOW_ATTRIBUTE(sfp_debug_update_state);
...
static void sfp_debugfs_init(struct sfp *sfp)
{
	...
	debugfs_create_file("update_state", 0600, sfp->debugfs_dir, sfp,
			    &sfp_debug_update_state_fops);
}
...
static bool sfp_los_event_active(struct sfp *sfp, unsigned int event)
{
	...
	/* Some SFP, e.g. SFP-GB-GE-T sn C1910065859, may have both los_normal
	 * and los_inverted unset, but is still capable of toggling LOS signal
	 * when cable is unplugged. In this case, assume HIGH is loss.
	 */
	return (los_options == los_inverted && event == SFP_E_LOS_LOW) ||
	       (los_options == los_normal && event == SFP_E_LOS_HIGH) ||
	       (los_options == 0 && event == SFP_E_LOS_HIGH);
}
...
static bool sfp_los_event_inactive(struct sfp *sfp, unsigned int event)
{
	...
	return (los_options == los_inverted && event == SFP_E_LOS_HIGH) ||
	       ...
	       (los_options == 0 && event == SFP_E_LOS_LOW);
}
...
static void sfp_apply_quirk_fixup(struct sfp *sfp)
{
	/* MDIO access to phy for FibreStore SFP-10G-T can't be performed via
	 * I2C addess 0x56. It seems to be accessed via SFP Enhanced Digital
	 * Diagnostics Interface via 0x51. However, due to lack of detailed
	 * information on Vendor Specific Locations and Vendor Specific Function
	 * Locations, there is no reliable way to access MDIO for the SFP.
	 * Disable phy probing for the SFP.
	 */
	if (!strncmp(sfp->id.base.vendor_name, "FS", strlen("FS")) &&
	    !strncmp(sfp->id.base.vendor_pn, "SFP-10G-T", strlen("SFP-10G-T"))) {
		sfp->id.base.extended_cc = 0;
	}
}
...
static int sfp_sm_mod_probe(struct sfp *sfp, bool report)
{
	...
	/* Print if Loss of Signal in extended option is not implemented */
	if (!(sfp->id.ext.options &
	    (cpu_to_be16(SFP_OPTIONS_LOS_INVERTED) | cpu_to_be16(SFP_OPTIONS_LOS_NORMAL))))
		dev_info(sfp->dev,
			 "EEPROM extended option shows Loss of Signal not implemented\n");
...
static void sfp_timeout(struct work_struct *work)
{
	...
	if(sfp->sm_state==SFP_S_WAIT_LOS && sfp->state & BIT(GPIO_LOS)) {
		sfp_check_state(sfp);
		sfp_sm_next(sfp, SFP_S_WAIT_LOS, msecs_to_jiffies(1000));
	}
}
