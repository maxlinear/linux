...
static int ppp_get_info(struct net_device *ppp_dev, struct ppp_channel **chan)
{
	...
	/*check ppp validity */
	if (unlikely(ppp->file.dead || refcount_read(&ppp->file.refcnt) == 0 ||
...
ppp_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* save secpath */
	if (xfrm_offload(skb)) {
		active_extensions = skb->active_extensions;
		ext = skb->extensions;
	...
	if (ext) {
		if (skb->active_extensions)
			refcount_dec(&ext->refcnt);
		skb->active_extensions = active_extensions;
		skb->extensions = ext;
	}
#endif
...
ppp_connect_channel(struct channel *pch, int unit)
{
	...
	pch->ppp = ppp;
	refcount_inc(&ppp->file.refcnt);
	if (pch->chan && pch->chan->ops->post_connect)
		pch->chan->ops->post_connect(pch->chan, ppp->dev, true);
	ppp_unlock(ppp);
	ret = 0;
...
ppp_disconnect_channel(struct channel *pch)
{
	...
	if (ppp) {
		...
		if (--ppp->n_channels == 0)
			wake_up_interruptible(&ppp->file.rwait);
		if (pch->chan && pch->chan->ops->post_connect)
			pch->chan->ops->post_connect(pch->chan, ppp->dev,
						     false);
		ppp_unlock(ppp);
		if (refcount_dec_and_test(&ppp->file.refcnt))
			ppp_destroy_interface(ppp);
