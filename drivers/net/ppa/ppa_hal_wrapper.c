...
uint32_t ppa_hsel_add_if(struct netif_info *ifinfo, uint32_t flag, uint32_t hal_id)
{
	if (!ppa_drv_hal_hook[hal_id])
		return PPA_FAILURE;

	return ppa_drv_hal_hook[hal_id](PPA_GENERIC_HAL_ADD_IF, ifinfo, flag);
}
EXPORT_SYMBOL(ppa_hsel_add_if);

uint32_t ppa_hsel_del_if(struct netif_info *ifinfo, uint32_t flag, uint32_t hal_id)
{
	if (!ppa_drv_hal_hook[hal_id])
		return PPA_FAILURE;

	return ppa_drv_hal_hook[hal_id](PPA_GENERIC_HAL_DEL_IF, ifinfo, flag);
}
EXPORT_SYMBOL(ppa_hsel_del_if);
...
/*****************************************************************************************
* ppacmd get HAL stats*
*****************************************************************************************/
uint32_t ppa_hsel_get_hal_stats(PPA_HAL_STATS *stat, uint32_t flag, uint32_t hal_id)
{
	if (hal_id >= MAX_HAL)
		return PPA_FAILURE;
	...
	return ppa_drv_hal_hook[hal_id](PPA_GENERIC_HAL_GET_STATS, (void *)stat, flag);
}

EXPORT_SYMBOL(ppa_hsel_get_hal_stats);
...
EXPORT_SYMBOL(ppa_hsel_get_acc_mode);
EXPORT_SYMBOL(ppa_hsel_set_acc_mode);
EXPORT_SYMBOL(ppa_hsel_hal_init);
EXPORT_SYMBOL(ppa_hsel_hal_exit);
