				...
				struct cpuidle_driver *drv, int index)
{
	...
	if (!strcmp(state->name, "C7"))
		epu_notifier_raw_chain(CPU_EVENT_C7_REQ_CORE(cpu), NULL);
...
static struct cpuidle_state lgm_cstates[] = {
	{
		.name = "C1",
		.desc = "MWAIT 0x00",
		.flags = MWAIT2flg(0x00),
...
static const struct x86_cpu_id intel_idle_ids[] __initconst = {
	...
	X86_MATCH_INTEL_FAM6_MODEL(ATOM_AIRMONT_NP,	&idle_cpu_lgm),
...
static int intel_idle_cpu_init(unsigned int cpu)
{
	...
	if (boot_cpu_data.x86_model == INTEL_FAM6_ATOM_AIRMONT_NP) {
		...
		rdmsrl_on_cpu(cpu, MSR_PKG_CST_CONFIG_CONTROL, &val);
		wrmsrl_on_cpu(cpu, MSR_PKG_CST_CONFIG_CONTROL, val | BIT(3));
	}
...
int intel_idle_disable_cstate(unsigned int cpu, int ena, int cstate_limit)
{
	struct cpuidle_device *dev;

	dev = per_cpu_ptr(intel_idle_cpuidle_devices, cpu);
	if (ena) {
		dev->cur_max_cstate = cstate_limit;
	} else {
		dev->cur_max_cstate = max_cstate;
	}
	return 0;
