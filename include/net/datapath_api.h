...
#define DP_MAX_UMT 3 /*!< maximum number of UMT port per DC
		      *   For LGM docsis, it can support up to 3
		      */
#else
...
#define DP_MAX_CTP_PER_DEV  34  /*!< @brief  max CTP per dev:
				 ...
				 *     in pcp mode
				 */
#define DP_MAX_DEQ_PER_SUBIF 32	/*!<@brief the maximum number of dequeue port per
				 ...
				 * only
				 */
#define DP_PMAPPER_DISCARD_CTP -1  /*!<@brief Discard ctp flag for pmapper*/
...
typedef struct dp_subif_single {
	...
	union {
		s16 subif; /*!< [in/out] Sub-interface Id as HW defined
			    */
		s16 subif_list[DP_MAX_CTP_PER_DEV]; /*!< [in/out] subif list
...
typedef struct dp_subif {
	...
	u32 flag_pmapper : 1; /*!< [out] flag to indicate whether this
			       ...
			       *   valid for dp_get_netif_subifid only
			       */
	u8 domain_id; /*!< [out] we support 32 domain per bridge only
			   * valid only DP_SUBIF_BR_DOMAIN is set in data_flag
			   */
	u32 domain_members; /*!< [out] one bit for one domain_id:
			     ...
			     * valid only if DP_SUBIF_BR_DOMAIN is set in data_flag
			     */
...
#if IS_ENABLED(CONFIG_DPM_DATAPATH_HAL_GSWIP32)
	union {
		u16 dfl_eg_sess[DP_DFL_SESS_NUM]; /*!< [out]
						   * default egress
						   * session id. This is
						   * for CPU TX to DC
						   * only
						   */
		u16 dfl_eg_sess_ext[DP_MAX_CTP_PER_DEV][DP_DFL_SESS_NUM];
						       /*!< [out]
							* default egress
							* session ids for
							* each gpid. This is
	...
	union {
		...
		u16 gpid_list[DP_MAX_CTP_PER_DEV];
	};
#endif
} dp_subif_t;
...
enum DP_SUBIF_DATA_FLAG {
	...
	DP_SUBIF_BR_DOMAIN = BIT(14), /*!< once this flag is set, it means
				       ...
				       */
	DP_SUBIF_CPU = BIT(15), /*!< Register special device as CPU port */
	DP_SUBIF_REINSERT = BIT(16), /*!< Register special device as reinsertion
				      * port
				      */
	DP_SUBIF_NO_US = BIT(17), /*!< Register device for those subif which
				   * are not required to support upstream traffic,
				   * for example: docsis mpeg and DBG subif.
				   */
	DP_SUBIF_TOE = BIT(18), /*!< Flag to tell whether subif_data->toe_disable value*/
...
struct dp_subif_data {
	...
	u32 domain_members; /*!< [in] one bit for one domain_id:
			     ...
			     */
	u8 toe_disable; /*!< [in] 0: to set toe related ops and capability for this device if valid
			 *	  1: keep original device toe ops/capability
			 */
};
...
enum dp_port_data_flag {
	...
	DP_F_DATA_PON_HGU_SEC_LOOP = BIT(10), /*!< Once this flag is set,
					       ...
					       */
	DP_F_REM_FCS = BIT(11), /*!< Once this flag is set, dp_rx need to
...
struct dp_port_data {
	...
	int deq_num;  /*!< [out] the number of dequeue port allocated for the
		       *         traffic to this device
		       */
	u32 link_speed_cap; /*!< [in] link speed capability, set by top level
			     *   network driver and used by CQM driver
			     */
	u32 n_wib_credit_pkt; /*!< [in] credit size for CQM non-DC egress port related to WIB
			       * If zero, set as HW capability
			       * Otherwise set as specified for all egress port linked to WIB
			       */
};
...
enum DP_DEV_DATA_FLAG {
	...
	DP_F_DEV_CONTINUOUS_Q = BIT(2), /*!< Reserve continuous Q for this dev
					 ...
					 */
	DP_F_DEV_EMODE = BIT(3),  /*!< GSWIP subif emode */
	DP_F_DEV_LOOKUP_MODE = BIT(4),  /*!< CQM qmap lookup mode */
};

/*! @brief enum CQE_LOOKUP_MODE for setting CQM queue lookup mode.
 *  The values have to match the values of DP enum CQE_LOOKUP_MODE.
 */
enum DP_CQE_LOOKUP_MODE {
	DP_CQE_LU_MODE0,		/* subif_id[13:8] + class[1:0] */
	DP_CQE_LU_MODE1,		/* subif_id[7:0]               */
	DP_CQE_LU_MODE2,		/* subif_id[11:8] + class[3:0] */
	DP_CQE_LU_MODE3,		/* subif_id[4:0]  + class[1:0] */
	DP_CQE_LU_MODE4,
	DP_CQE_LU_MODE5,
	DP_CQE_LU_MODE6,
	DP_CQE_LU_MODE7,
	DP_CQE_LU_MODE_INVALID,
};
...
/*! @brief struct dp_rx_ring_egp, which is used for DirectConnected (DC)
 *  applications
 */
struct dp_rx_ring_egp {
	int deq_port;       /*!< EGP port/dequeue port ID */
	int tx_ring_size;   /*!< ring size */
	u32 tx_pkt_credit;  /*!< port tx packet credit */
	void *txpush_addr;  /*!< QOS TX push physical address */
	void *txpush_addr_qos; /*!< QOS TX push physical address for
...
struct dp_rx_ring {
	...
	/*!< [out] number of egp for rxout ring in QOS mode
	 *         For QOS bypass mode, CQM driver must keep num_egp zero
	 *         So far DPM regards all these egp can be shared by other DC,
	 *         like QOS bypass mode, 8 CQM internal queue are shared by DC
	 */
	int num_egp;
	/*!< [out] cqm deque port info for this rxout ring.
	 *         Valid if num_egp not zero only
	 *         dpm use it for CQM queue lookup mapping
	 */
	struct dp_rx_ring_egp egp[DP_RX_RING_DEQ_NUM];
	enum DP_BPRESS bpress_out; /*!< [in] flag to enable/disable back
				    * pressure for rxout ring
				    */
	enum DP_BPRESS bpress_alloc; /*!< [in] flag to enable/disable back
				      * pressure for rxin alloc ring
				      */
};
...
struct dp_tx_ring {
	...
	bool not_valid; /*!< [in] flag to indicate whether to use this DC dequeue port (TXIN ring)
			  * DPM use it to decide which egress port to be used:
			  * true  - use the egress port returned by cbm_dp_port_alloc (WIB port)
			  * false - use the egress port returned by cbm_dp_port_alloc_complete (DC port)
			  */
	enum DP_BPRESS bpress_in; /*!< [in] flag to enable/disable back pressure
				   * for txin ring
				   */
	enum DP_BPRESS bpress_free; /*!< [in] flag to enable/disable back pressure
				     * for txout free ring
				     */
};

/*! @brief struct umt_dc_map, used for DirectConnected (DC)
 *  applications
 */
struct umt_dc_map {
	s16 dc_idx;  /*!< [in] relative dc index for this umt */
	bool f_not_alloc; /*!< [in] false: need to alloc UMT port (By default),
			   *	    true: no need to alloc UMT port
			   */
};
 ...
 *  applications
 */
struct dc_extra_info {
        bool no_bm_dummy_read; /*!< [in] false: need CQM dummy buffer read
				*        true: no need CQM dummy buffer read
				*/
};
...
struct dp_dev_data {
	...
	struct dp_tx_ring tx_ring[DP_TX_RING_NUM]; /*!< [in/out] DC tx ring info
						    *   Make sure next tx_ring's
						    *   tx_deq_port bigger
						    *   than the previous
						    *   tx_ring's tx_deq_port
						    */
	...
	struct dc_extra_info dc_info[DP_DC_NUM]; /*!< [in] to specify any extra
						  * DC information
						  */
	struct umt_dc_map umt_dc[DP_MAX_UMT]; /*!< [in] map UMT to DC port
					       * Later DPM fetch the dma/cqm
					       * port info
					       * from ring[umt_dc[x].dc_idx] to
					       * umt
					       */
	u32 enable_cqm_meta : 1; /*!< enable CQM buffer meta data marking */

	u16 max_subif;  /*!< [in] maximum subif required for this dp port */
	u16 max_ctp;    /*!< [in] maximum ctp required which will be mapped to
...
struct dp_spl_egp {
	...
	int qid;  /*!< [out] physical qid to the EGP
		   ...
		   *           PP NF: valid
		   */
	enum DP_BPRESS bpress_in; /*!< [in] flag to enable/disable back
				   * pressure for tx in ring
				   */
	enum DP_BPRESS bpress_free; /*!< [in] flag to enable/disable back
				     * pressure for tx out free ring
				     */
};
...
/*!
 *@brief dp_set_net_dev_ops_priv API
 *@param[in] dev: pointer to net_device structure
 *@param[in] ops_callback: pointer to new callback,
 *                         will be ignore if DP_OPS_RESET is set in flag
 *@param[in] ops_offset: offset of the new callback,
 ...
 *@return DP_SUCCESS on succeed and DP_FAILURE on failure
 */
int dp_set_net_dev_ops_priv(struct net_device *dev,	const void *ndo_cb,
		const int offset, u32 flag, const char *memo);
...
/*!
 *@brief dp_pce_rmon_get Reserve RMON counter id
 *@param[out] id: pointer to reserved id
 *@return DP_SUCCESS on succeed and DP_FAILURE on failure
 */
int dp_pce_rmon_get(int *id);

/*!
 *@brief dp_pce_rmon_get Release RMON counter id
