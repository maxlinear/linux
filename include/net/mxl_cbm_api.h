...
#define CQM_POLICY_NUM       257
#define CQM_POOL_NUM       18
 ...
 * note CBM Port can be deallocated only after the port is Uninitialized
 */
int32_t
...
struct cbm_dflt_resv {
	...
	u32 f_cpu; /* [in]normal CPU GPID if it is set*/
	u32 f_spl_gpid; /* [in] special GPID if it is set */
	u32 f_segment; /* [in] segmentation buffer if it is set */
...
struct cbm_mtu {
	...
	int alloc_flag;	/* [in] */
	int subif_flag;	/* [in] */
	enum DP_SPL_TYPE spl_conn_type; /* [in] for CPU Special conn only */
	u32 f_cpu; /* [in] normal CPU GPID if it is set */
	u32 f_spl_gpid; /* [in] special GPID if it is set */
	u32 f_segment; /* [in] segmentation buffer if it is set */
	int mtu;	/* [out] */
...
#define CQM_UP_STATS(atomic) atomic64_add(1, &(atomic))
#else
#define CQM_UP_STATS(atomic)
#endif
