...
/** \brief Traffic class to Reduced Traffic class configuration */
typedef struct {
	/** Reduced Traffic class value (0 - 3).
	    Array Index is Traffic class */
	u8	nRdTrafficClass[16];
} GSW_QoS_RdTraffic_ClassCfg_t;
...
/** \brief Ethernet port interface mode.
    ...
    Used by \ref GSW_portLinkCfg_t.
    The supporrted modes on PRX and LGM platforms are only GSW_PORT_HW_GMII and
    GSW_PORT_HW_XGMII */
typedef enum {
...
typedef struct {
	ltq_bool_t	bVlanRmonEnable;
	ltq_bool_t	bIncludeBroadCastPktCounting;
	u32	nVlanLastEntry;
...
/**
 ...
 This structure contains the RMON counters of an CTP.
    Used by \ref GSW_RMON_CTP_GET. */
typedef struct {
	/** CTP number (zero-based counting). The valid range is hardware
	    dependent. An error code is delivered if the selected CTP is not
	    available. This parameter specifies for which CTP the RMON
	    counter is read. It has to be set by the application before
	    calling \ref GSW_RMON_CTP_GET. */
	u16	nCTP;
	/** Tx Bytes. */
	u64	nTxBytes;
	/** Tx Packet. */
...
typedef struct {
	...
	ltq_bool_t bVlanTagSelectionEnable;
...
typedef struct {
	...
	ltq_bool_t bIngressVlanBasedMacLearningEnable;
	...
	ltq_bool_t bVlanSrcMacVidEnable;
	/** 0 - Disable, VLAN Priority field is not used
	    ...
	    filtering */
	ltq_bool_t bVlanDstMacPriorityEnable;
	/** 0 - Disable, VLAN CFI/DEI field is not used
	    ...
	    destination MAC address look up and
	    filtering. */
	ltq_bool_t bVlanDstMacDEIEnable;
	/** 0 - Disable, VLAN ID field is not used and
...
typedef struct {
	...
	u32 nBridgePortId;
	/** nvalidMSB: To configure Valid bits for SubInterface id in non-WLAN mode. */
	u32 nValidMSB;
...
/**
   ...
   \param GSW_RMON_CTP_cnt_t  Pointer to pre-allocated
   \ref GSW_RMON_CTP_cnt_t structure. The structure element 'nCTP' is
   an input parameter that describes from which CTP to read the RMON counter.
   All remaining structure elements are filled with the counter values.

   \remarks The function returns an error in case the given 'nCTP' is
   out of range.
   ...
   - An error code in case an error occurs
*/
#define GSW_RMON_CTP_GET	_IOWR(GSW_RMON_MAGIC, 0xD, GSW_RMON_CTP_cnt_t)
...
/**
	\brief Following are for GSWIP 3.2 VLAN RMON operation

	\param GSW_VlanCounterMapping_config_t Pointer to \ref GSW_VlanCounterMapping_config_t.
	\param GSW_VLAN_RMON_control_t Pointer to \ref GSW_VLAN_RMON_control_t.

*/
#define GSW_VLAN_COUNTER_MAPPING_SET 		_IOWR(GSW_VLANCOUNTER_MAGIC, 0x01, GSW_VlanCounterMapping_config_t)
#define GSW_VLAN_COUNTER_MAPPING_GET 		_IOWR(GSW_VLANCOUNTER_MAGIC, 0x02, GSW_VlanCounterMapping_config_t)
#define GSW_VLAN_RMON_GET				 	_IOWR(GSW_VLANCOUNTER_MAGIC, 0x03, GSW_VLAN_RMON_cnt_t)
#define GSW_VLAN_RMON_CONTROL_SET 			_IOWR(GSW_VLANCOUNTER_MAGIC, 0x04, GSW_VLAN_RMON_control_t)
#define GSW_VLAN_RMON_CONTROL_GET		 	_IOWR(GSW_VLANCOUNTER_MAGIC, 0x05, GSW_VLAN_RMON_control_t)
#define GSW_VLAN_RMON_CLEAR		 	        _IOWR(GSW_VLANCOUNTER_MAGIC, 0x06, GSW_VLAN_RMON_cnt_t)
