...
enum tc_codel_command {
	TC_CODEL_REPLACE,
	TC_CODEL_DESTROY,
	TC_CODEL_STATS,
};

struct tc_codel_qopt_offload_params {
	u32 target;
	u32 ce_threshold;
	u32 interval;
	u32 mtu;
...
struct tc_codel_qopt_offload {
	enum tc_codel_command command;
	u32 handle;
	u32 parent;
	union {
		struct tc_codel_qopt_offload_params cparams;
	};
};
