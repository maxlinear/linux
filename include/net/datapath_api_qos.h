...
enum dp_q_drop_mode {
	...
	DP_QUEUE_DROP_CODEL,  /*!< codel mode */
};
...
#define DP_QFLUSH_FLAG_RESTORE_LOOKUP        BIT(0) /*!< @brief Restore original
						     * lookup queue mapping
						     * after flushing.
						     */
#define DP_QFLUSH_FLAG_RESTORE_QOS_PORT      BIT(1) /*!< @brief Restore queues
						     * to original deq port
						     * after flushing, otherwise
						     * they will be kept under
						     * drop port.
						     */
#define DP_QFLUSH_FLAG_CONTINUE_BLOCK_QUEUE  BIT(2) /*!< @brief Keep queues
						     * in block mode
						     * after flushing.
						     */
#define DP_QFLUSH_FLAG_NO_LOCK  BIT(3) /*!< @brief Skip the lock if it is
						     * already acquired.
						     */
 ...
 *   @param [in/out] cfg
 *   @param [in] flag: DP_QFLUSH_FLAG parameter
 *   @return failure DP_FAILURE
 *           succeed DP_SUCCESS
 */
...
struct dp_hif_datapath {
       ...
       struct dp_egress eg[DP_NUM_HIF_PRIO];
