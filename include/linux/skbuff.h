...
{
#if IS_ENABLED(CONFIG_MXL_CBM_SKB)
	if (!cqm_page_ref(page_to_phys(skb_frag_page(frag)) + frag->bv_offset))
		return;
#endif
...
#if IS_ENABLED(CONFIG_MXL_CBM_SKB)
	if (!cqm_page_unref(page_to_phys(skb_frag_page(frag)) + frag->bv_offset))
		return;
#endif
...
enum skb_ext_id {
	...
	SKB_EXT_MPTCP,
#endif
#if IS_ENABLED(CONFIG_ARCH_SKB_EXT)
#define SKB_EXT_ARCH_EXT_SIZE SZ_1K
	SKB_EXT_ARCH,
#endif
