#ifndef _UAPI_PPA_API_H
#define _UAPI_PPA_API_H
/*******************************************************************************
 **
 ** FILE NAME	: ppa_api.h
 ** PROJECT	: PPA
...
#define SESSION_LIST_HASH_SHIFT 11
#define SESSION_LIST_HASH_BIT_LENGTH 12
#define SESSION_LIST_HASH_MASK ((1 << SESSION_LIST_HASH_BIT_LENGTH) - 1)
#define SESSION_LIST_HASH_TABLE_SIZE (1 << (SESSION_LIST_HASH_BIT_LENGTH + 1)) /*8K hash entries*/
 ...
 */
#define PPA_MAX_VLAN_FILTER 32 /*!< Maximum number of VLAN filter */
/*!
	\brief PPA_IOC_MAGIC
 */
...
#define MAX_HALS 8
/*!
	\brief MAX_HAL_NAME_LEN
 */
#define MAX_HAL_NAME_LEN 32
/*
 * flags
 */
/*!
...
#define SESSION_FLAG2_RXCSUM_VALID		0x00020000
...
/*
 *For LAN-WAN Acceleration Control command
 */
#define PPA_LAN_VALID  (1 << 0)
#define PPA_WAN_VALID  (1 << 1)

#define PPA_IS_LAN_VALID(flags) (flags & PPA_LAN_VALID)
#define PPA_IS_WAN_VALID(flags) (flags & PPA_WAN_VALID)
...
typedef struct {
	...
	PPA_IPADDR ip; /*!< multiple ip address format support */
} IP_ADDR_C;
/*!
...
typedef struct {
	...
	uint32_t group_id; /*!< Group id for multicast entry */
} PPA_CMD_MC_ENTRY;
/*!
...
typedef struct {
	...
	uint32_t drop; /*!< Flag to specify drop event */
...
typedef struct {
	...
	uint32_t		pir; /*!< Peak Information Rate in bytes/s */
	uint32_t		pbs; /*!< Peak Burst Size */
	uint32_t		meterid; /*!< Meter ID Configured on system*/
	uint32_t		flags; /*!< Flags define operations on meters*/
} PPA_QOS_METER_INFO;
/*!
...
/*!
	\brief This is the data structure for axhals info and status
 */
typedef struct {
	char name[MAX_HAL_NAME_LEN]; /*!< Used to store hal names */
	int status; /*!< Used to store status of each hal */
} ppa_name_values;
/*!
	\brief This is the data structure for PPA Hal Config Info
 */
typedef struct {
	uint32_t status; /*!< status to show the all hals */
	uint32_t enable; /*!< enable to either enable/diable hal */
	char hal[MAX_HAL_NAME_LEN]; /*!< Used to store the hal name */
	uint32_t flags; /*!< reserved for future */
	ppa_name_values axhals[MAX_HALS]; /*!< Used to store hal and respective hal_id */
} PPA_CMD_HAL_CONFIG_INFO;
/*!
	\brief This is the data structure for PPA HAL Stats Info
 */
typedef struct {
	uint32_t max_uc; /*!< Max number of uc session HAL can accelerate */
	uint32_t max_mc; /*!< Max number of mc session HAL can accelerate */
	uint32_t max_uc_ipv4; /*!< Max number of IPv4 uc session HAL can accelerate */
	uint32_t max_uc_ipv6; /*!< Max number of IPv6 uc session HAL can accelerate */
	uint32_t curr_uc_ipv4; /*!< current number of IPv4 uc session that HAL has accelerated */
	uint32_t curr_uc_ipv6; /*!< current number of IPv6 uc session that HAL has accelerated */
	uint32_t curr_mc_ipv4; /*!< current number of IPv4 mc session that HAL has accelerated */
	uint32_t curr_mc_ipv6; /*!< current number of IPv6 mc session that HAL has accelerated */
	uint32_t tot_uc_drop; /*!< total number of uc session dropped by HAL */
	uint32_t tot_mc_drop; /*!< total number of uc session dropped by HAL */
} PPA_HAL_ALL_STATS;
typedef struct {
	uint32_t get_stats; /*!< to show stats of an hal */
	char hal[MAX_HAL_NAME_LEN]; /*!< Used to store the hal name */
	ppa_name_values axhals[MAX_HALS]; /*!< Used to store hal and respective hal_id */
	PPA_HAL_ALL_STATS all_hal_stats; /*!< used to store all stats */
} PPA_CMD_HAL_STATS_INFO;
/*!
...
typedef union {
	...
	PPA_CMD_MC_GROUP_INFO mc_sess_info; /*!< PPA multicast session parameter */
	...
	PPA_CMD_HAL_CONFIG_INFO hal_info; /*!< PPA hal info. */
	PPA_CMD_HAL_STATS_INFO halstats_info; /*!< PPA hal stats info*/
} PPA_CMD_DATA;
/*@}*/ /* PPA_IOCTL */
/* -------------------------------------------------------------------------- */
...
typedef enum {
	...
	PPA_CMD_DEL_MC_NR, /*!< NR for PPA_CMD_DEL_MC */
	...
	PPA_CMD_HAL_CONFIG_NR, /*!< NR for PPA_CMD_HAL_CONFIG */
	PPA_CMD_HAL_STATS_NR, /*!< NR for PPA_CMD_HAL_STATS*/
	PPA_CMD_COUNT_IF_NR, /*!< NR for interface count */
	/* PPA_IOC_MAXNR should be the last one in the enumeration */
...
/** PPA Del a Multicast session Command. Value is manipulated by _IOR() macro for final value
	\param PPA_CMD_MC_GROUP_INFO The parameter points to a
	\ref PPA_CMD_MC_GROUP_INFO structure
 */
#define PPA_CMD_DEL_MC _IOR(PPA_IOC_MAGIC, PPA_CMD_DEL_MC_NR, PPA_CMD_MC_GROUP_INFO)
 ...
 *      */
#define PPA_CMD_SET_QOS_METER \
	_IOW(PPA_IOC_MAGIC, PPA_CMD_SET_QOS_METER_NR, PPA_QOS_METER_INFO)
...
/** PPA Get PPA Hash Summary. Value is manipulated by _IOWR() macro for final value
	...
	\ref PPA_CMD_COUNT_INFO structure
 */
/** PPA hal variable values
	\PPA_CMD_HAL_CONFIG, The parameter points to a
	\ref PPA_CMD_HAL_INFO structure
 */
#define PPA_CMD_HAL_CONFIG		_IOWR(PPA_IOC_MAGIC, PPA_CMD_HAL_CONFIG_NR, PPA_CMD_HAL_CONFIG_INFO)
/** PPA halstats variable values
	\param PPA_CMD_HAL_STATS_INFO. The parameter points to a
	\ref PPA_CMD_HAL_STATS_INFO structure
*/
#define PPA_CMD_HAL_STATS		_IOR(PPA_IOC_MAGIC, PPA_CMD_HAL_STATS_NR, PPA_CMD_HAL_STATS_INFO)

/*! \brief Get the PPA interface count.
 *  \param PPA_CMD_COUNT_INFO. The parameter points to
 *  \ref PPA_CMD_COUNT_INFO structure.
 */
#define PPA_CMD_GET_IF_COUNT \
	_IOR(PPA_IOC_MAGIC, PPA_CMD_COUNT_IF_NR, PPA_CMD_COUNT_INFO)
/*@}*/ /* PPA_IOCTL */
